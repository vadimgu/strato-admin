...
const replaceImport = (root, j) => {
  ...
  const datagridImport = reactAdminImport.filter((path) => {
    return path.node.specifiers.some(
      (specifier) => j.ImportSpecifier.check(specifier) && specifier.imported.name === 'Datagrid',
    );
  });
  ...
  reactAdminImport.replaceWith(({ node }) =>
    j.importDeclaration(
      node.specifiers.map((specifier) => {
        if (j.ImportSpecifier.check(specifier) && specifier.imported.name === 'Datagrid') {
...
const cleanAttributes = (node, j) => {
  ...
  const sxRenamedAttributes = rowSxRenamedAttributes.map((attr) => {
    if (j.JSXAttribute.check(attr) && attr.name.name === 'sx' && j.JSXExpressionContainer.check(attr.value)) {
      const expression = attr.value.expression;
      if (j.ObjectExpression.check(expression)) {
        expression.properties.map((prop) => {
          if (j.ObjectProperty.check(prop) && j.Literal.check(prop.key) && typeof prop.key.value === 'string') {
            prop.key.value = prop.key.value.replace(/RaDatagrid-/g, 'RaDataTable-');
          }
          return prop;
        });
        return attr;
...
const transformChildren = (root, j) => {
  ...
  datagridComponents.forEach((dataTableComponent) => {
    const children = dataTableComponent.value.children.filter((child) => j.JSXElement.check(child));
...
const transformChild = (root, j, child) => {
  ...
  if (
    ...
    !child.openingElement.attributes.some(
      (attr) => j.JSXAttribute.check(attr) && !['source', 'label', 'empty'].includes(attr.name.name),
    )
  ) {
    ...
    !child.openingElement.attributes.some(
      (attr) =>
        j.JSXAttribute.check(attr) && !['source', 'label', 'empty', 'options', 'locales'].includes(attr.name.name),
    )
  ) {
    ...
    root
      ...
      .forEach((dataTableComponent) => {
        dataTableComponent.value.children = dataTableComponent.value.children.map((c) => (c === child ? newChild : c));
      });
  }
};
...
const wrapChild = (j, child) => {
  ...
  return j.jsxElement(
    j.jsxOpeningElement(
      ...
      labelAttribute ? [labelAttribute] : sourceAttribute ? [sourceAttribute] : [],
      false,
    ),
...
const cleanImports = (root, j) => {
  ...
  const textFieldImport = imports.filter((path) => {
    return path.node.specifiers.some(
      (specifier) => j.ImportSpecifier.check(specifier) && specifier.imported.name === 'TextField',
    );
  });
  const numberFieldImport = imports.filter((path) => {
    return path.node.specifiers.some(
      (specifier) => j.ImportSpecifier.check(specifier) && specifier.imported.name === 'NumberField',
    );
  });
