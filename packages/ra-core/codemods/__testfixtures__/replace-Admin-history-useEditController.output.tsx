...
describe('useEditController', () => {
  ...
  it('should decode the id from the route params', async () => {
    const getOne = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: { id: 'test?', title: 'hello' } }));
    const dataProvider = { getOne } as unknown as DataProvider;
        ...
        <CoreAdminContext dataProvider={dataProvider}>
          <Routes>
            <Route
              path="/posts/:id"
              element={
                <EditController resource="posts">{({ record }) => <div>{record && record.title}</div>}</EditController>
              }
            />
          </Routes>
        </CoreAdminContext>
  ...
  it('should allow to register middlewares', async () => {
    ...
    const middleware: Middleware<ReturnType<typeof useUpdate>[0]> = jest.fn((resource, params, options, next) => {
      return next(resource, { ...params, meta: { addedByMiddleware: true } }, options);
    });

    const Child = () => {
      useRegisterMutationMiddleware<ReturnType<typeof useUpdate>[0]>(middleware);
      return null;
    };
    render(
      <CoreAdminContext dataProvider={dataProvider}>
