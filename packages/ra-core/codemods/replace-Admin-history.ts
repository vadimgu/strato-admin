...
const adminComponentsWithHistoryPropRemoved = ['Admin', 'AdminContext', 'CoreAdmin', 'CoreAdminContext'];
...
module.exports = (file, api: j.API) => {
  ...
  if (reactAdminImport.length) {
    ...
    reactAdminImport.replaceWith(({ node }) =>
      j.importDeclaration([...(node.specifiers || []), importSpecifier], node.source),
    );
  } else {
    ...
    root
      ...
      .insertAfter(j.importDeclaration([importSpecifier], j.literal('react-admin')));
  }
          ...
          .replaceWith(({ node }) =>
            j.jsxElement(
              j.jsxOpeningElement(
                ...
                initialEntries
                  ? [j.jsxAttribute(j.jsxIdentifier('initialEntries'), j.jsxExpressionContainer(initialEntries))]
                  : [],
                false,
              ),
              j.jsxClosingElement(j.jsxIdentifier('TestMemoryRouter')),
              [j.jsxText('\n'), node, j.jsxText('\n')],
            ),
          );
      });
