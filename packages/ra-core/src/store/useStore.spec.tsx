...
describe('useStore', () => {
  ...
  const StoreReader = ({ name, defaultValue }: { name: string; defaultValue?: any }) => {
  ...
  it('should accept an updater function as parameter', async () => {
    const UpdateStore = () => {
      const [, setValue] = useStore<string>('foo.bar');
      return <button onClick={() => setValue((current) => `${current} world`)}>update</button>;
    };
    render(
      <StoreContextProvider value={memoryStore({ 'foo.bar': 'hello' })}>
        <StoreReader name="foo.bar" />
    ...
    const MyComponent = () => {
      ...
      return (
        <StoreContextProvider value={memoryStore({})}>
          ...
          <button onClick={() => setStoreKey('list2')}>change key</button>
        </StoreContextProvider>
      );
    };
    render(<MyComponent />);
