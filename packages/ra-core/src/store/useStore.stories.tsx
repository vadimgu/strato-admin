...
const StoreRaw = ({ name }) => {
  const [value] = useStore(name);
  return <>{value === undefined ? 'undefined' : value === null ? 'null' : value === '' ? "''" : value}</>;
};
...
export const UIStore = () => {
  ...
  return (
    <>
      ...
      <input type="range" min="10" max="45" value={fontSize} onChange={(e) => setSize(parseInt(e.target.value, 10))} />
      <h1 style={{ fontSize }}>Lorem ipsum dolor sit amet, consectetur adipiscing elit.</h1>
    </>
  );
};
