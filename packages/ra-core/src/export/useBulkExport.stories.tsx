...
import { CoreAdminContext, Exporter, ListBase, useBulkExport, UseBulkExportOptions, useListContext } from '..';
...
const dataProvider = fakeRestProvider(data, process.env.NODE_ENV !== 'test', 300);

export const Basic = ({
  exporter = (data, fetchRelatedRecords, dataProvider, resource) => {
    alert(`Exporting ${data.length} items (${data.map((record) => record.id).join(', ')}) from ${resource}`);
  },
}: {
  exporter?: Exporter;
}) => (
...
const ListView = () => {
  const { data, error, isPending, selectedIds, onToggleItem } = useListContext();
