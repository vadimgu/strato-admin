...
const TestedComponent = ({ role }) => {
  return (
    <TestMemoryRouter>
      <CoreAdminContext>
        <CoreAdminRoutes layout={MyLayout} catchAll={CatchAll} loading={Loading}>
          <Resource name="posts" />
...
const ResourceWithPermissions = (props: ResourceProps) => <Resource {...props} />;
...
const TestedComponentWithPermissions = () => {
  ...
  return (
    <TestMemoryRouter>
      <CoreAdminContext authProvider={authProvider}>
        <CoreAdminRoutes layout={MyLayout} catchAll={CatchAll} loading={Loading}>
          <ResourceWithPermissions name="posts" list={<div />} create={<div />} edit={<div />} show={<div />} />
          <ResourceWithPermissions name="comments" list={<div />} create={<div />} edit={<div />} show={<div />} />
          <ResourceWithPermissions name="users" list={<div />} create={<div />} edit={<div />} show={<div />} />
        </CoreAdminRoutes>
      </CoreAdminContext>
    </TestMemoryRouter>
  );
...
const TestedComponentWithOnlyLazyCustomRoutes = ({ navigateCallback }) => {
  ...
  return (
    <TestMemoryRouter navigateCallback={navigateCallback}>
      <CoreAdminContext>
        <CoreAdminRoutes layout={MyLayout} catchAll={CatchAll} loading={Loading} ready={Ready}>
          {lazyRoutes}
        </CoreAdminRoutes>
      </CoreAdminContext>
    </TestMemoryRouter>
...
const TestedComponentWithForcedRoutes = () => {
  return (
    <TestMemoryRouter>
      <CoreAdminContext>
        <CoreAdminRoutes layout={MyLayout} catchAll={CatchAll} loading={Loading}>
          <Resource name="posts" list={<div />} hasCreate hasEdit hasShow />
...
describe('useConfigureAdminRouterFromChildren', () => {
  it('should always load static resources', async () => {
    ...
    expect(screen.queryByText(`"name":"user"`, { exact: false })).toBeNull();
    expect(screen.queryByText(`"name":"admin"`, { exact: false })).toBeNull();
  });
  it('should not call the children function until the permissions have been retrieved', async () => {
    const callback = jest.fn(() => Promise.resolve((resolve) => setTimeout(resolve, 50)));
