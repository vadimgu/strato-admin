  ...
  describe('With children returned from a function as children', () => {
      ...
      render(
        ...
        >
          <CoreAdminContext dataProvider={testDataProvider()}>
            <CoreAdminRoutes layout={Layout} catchAll={CatchAll} loading={Loading}>
    ...
    it('should show the first allowed resource by default when there is an authProvider that supports canAccess', async () => {
      const authProvider = {
        ...
        checkError: jest.fn().mockResolvedValue(''),
        getPermissions: jest.fn().mockResolvedValue(''),
        canAccess: jest.fn(({ resource }) => Promise.resolve(resource === 'comments')),
      };
      render(<Basic authProvider={authProvider} />);
      await screen.findByText('CommentList');
    });
    ...
    it('should return loading while the function child is not resolved', async () => {
      ...
      render(
        ...
        >
          <CoreAdminContext authProvider={authProvider} dataProvider={testDataProvider()}>
            <CoreAdminRoutes layout={Layout} loading={Loading} catchAll={CatchAll}>
              <CustomRoutes noLayout>
  ...
  describe('anonymous access', () => {
    it('should not wait for the authProvider.checkAuth to return before rendering by default', () => {
      ...
      render(
        <TestMemoryRouter>
          <CoreAdminContext authProvider={authProvider} dataProvider={testDataProvider()}>
            <CoreAdminRoutes {...defaultProps} layout={Layout} loading={Loading} catchAll={CatchAll}>
    ...
    it('should render custom routes with no layout when the user is not authenticated ', async () => {
      ...
      await waitFor(() => expect(screen.queryByText('Custom')).not.toBeNull());
    });
  });
  describe('requireAuth', () => {
    it('should wait for the authProvider.checkAuth to return before rendering when requireAuth is true', async () => {
      ...
      const authProvider = {
        login: jest.fn().mockResolvedValue(''),
        logout: jest.fn().mockResolvedValue(''),
        checkAuth: (): Promise<void> => new Promise((res) => (resolve = res)),
        checkError: jest.fn().mockResolvedValue(''),
        getPermissions: jest.fn().mockResolvedValue(''),
      };
      ...
      render(
        <TestMemoryRouter>
          <CoreAdminContext authProvider={authProvider} dataProvider={testDataProvider()}>
            <CoreAdminRoutes layout={Layout} loading={Loading} catchAll={CatchAll} requireAuth>
              <Resource name="posts" list={() => <i>PostList</i>} />
            </CoreAdminRoutes>
          </CoreAdminContext>
        </TestMemoryRouter>,
      ...
      resolve();
      await waitFor(() => expect(screen.queryByText('PostList')).not.toBeNull());
    });
    it('should redirect anonymous users to login when requireAuth is true and user accesses a resource page', async () => {
      ...
      const authProvider = {
        login: jest.fn().mockResolvedValue(''),
        logout: jest.fn().mockResolvedValue(''),
        checkAuth: (): Promise<void> => new Promise((res, rej) => (reject = rej)),
        checkError: jest.fn().mockResolvedValue(''),
        getPermissions: jest.fn().mockResolvedValue(''),
      };
    ...
    it('should redirect anonymous users to login when requireAuth is true and user accesses a custom route', async () => {
      ...
      render(
        <TestMemoryRouter initialEntries={['/custom']}>
          <CoreAdminContext dataProvider={testDataProvider()} authProvider={authProvider}>
            <CoreAdminRoutes layout={Layout} loading={Loading} catchAll={CatchAll} requireAuth>
              <CustomRoutes>
                <Route path="/custom" element={<i>Custom</i>} />
              </CustomRoutes>
