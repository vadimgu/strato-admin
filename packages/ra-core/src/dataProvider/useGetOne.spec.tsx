...
describe('useGetOne', () => {
  ...
  beforeEach(() => {
    dataProvider = testDataProvider({
      getOne: jest.fn().mockResolvedValue({ data: { id: 1, title: 'foo' } }),
    });
  });
  ...
  it('should not call dataProvider.getOne() on mount if enabled is false', async () => {
    const { rerender } = render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetOne resource="posts" id={1} options={{ enabled: false }} />
      </CoreAdminContext>,
    );
    ...
    rerender(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetOne resource="posts" id={1} options={{ enabled: true }} />
      </CoreAdminContext>,
    );
    await new Promise((resolve) => setTimeout(resolve));
    expect(dataProvider.getOne).toHaveBeenCalledTimes(1);
  ...
  it('should set the error state when the dataProvider fails', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetOne resource="posts" id={1} callback={hookValue} options={{ retry: false }} />
      </CoreAdminContext>,
    );
  ...
  it('should not execute success side effect on error on refetch', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetOne resource="posts" id={1} options={{ onSuccess, onError, retry: false }} callback={callback} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getOne).toHaveBeenCalledTimes(1);
  ...
  it('should execute error side effects on failure', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetOne resource="posts" id={1} options={{ onError, retry: false }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getOne).toHaveBeenCalledTimes(1);
