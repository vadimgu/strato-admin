...
describe('useInfiniteGetList', () => {
  const UseInfiniteGetList = ({
    ...
    const hookValue = useInfiniteGetList(resource, { pagination, sort, filter, meta }, options);
  ...
  it('should call the dataProvider on update when the resource changes', async () => {
    ...
    const { rerender } = render(<Basic dataProvider={dataProvider} resource="heroes" />);
    await waitFor(() => {
      expect(dataProvider.getList).toBeCalledTimes(1);
    });
    rerender(<Basic dataProvider={dataProvider} />);
  ...
  it('should call success side effects on success', async () => {
      ...
      getList: (resource, params) => {
        return Promise.resolve({
          data: countries.slice(
            ...
            (params.pagination.page - 1) * params.pagination.perPage + params.pagination.perPage,
          ),
    ...
    render(
      <Basic dataProvider={dataProvider} pagination={{ page: 1, perPage: 1 }} options={{ onSuccess: onSuccess1 }} />,
    );
    await waitFor(async () => {
      expect(onSuccess1).toBeCalledTimes(1);
      expect(screen.getByLabelText('country').innerHTML).toContain('France');
  ...
  it('should not pre-populate getOne Query Cache if more than 100 results', async () => {
    ...
    render(
      <CoreAdminContext queryClient={queryClient} dataProvider={dataProvider}>
        <UseInfiniteGetList callback={callback} pagination={{ page: 1, perPage: 101 }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
    ...
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toBeUndefined();
  });
  ...
  it('should not pre-populate getOne Query Cache if more than 100 results across several pages', async () => {
    ...
    render(
      <CoreAdminContext queryClient={queryClient} dataProvider={dataProvider}>
        <UseInfiniteGetList callback={callback} pagination={{ page: 1, perPage: 51 }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(
    ...
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toBeDefined();
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '51' }])).toBeDefined();
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '52' }])).not.toBeDefined();
    // Fetch next page
  ...
  it('should only populate the getOne Query Cache with the records from the last fetched page', async () => {
    ...
    render(
      <CoreAdminContext queryClient={queryClient} dataProvider={dataProvider}>
        <UseInfiniteGetList callback={callback} pagination={{ page: 1, perPage: 1 }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(
    ...
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toEqual({ id: 1, title: 'item 1' });
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '2' }])).toBeUndefined();
    ...
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '2' }])).toEqual({ id: 2, title: 'item 2' });
    // Check that the getOne Query Cache for item 1 has not been overriden
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toEqual({ id: 1, title: 'changed!' });
  });
