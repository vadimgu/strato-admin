...
describe('useDataProvider', () => {
  ...
  it('should return a way to call the dataProvider', async () => {
    const getOne = jest.fn(() => Promise.resolve({ data: { id: 1, title: 'foo' } }));
    ...
    expect(queryByTestId('data').textContent).toBe('{"id":1,"title":"foo"}');
  });
  ...
  it('should display a meaningful error when the dataProvider throws a sync error', async () => {
    ...
    await screen.findByText('The dataProvider threw an error. It should return a rejected Promise instead.');
    c.mockRestore();
  });
  ...
  it('should call getList and not show error', async () => {
    const getList = jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'foo' }], total: 1 }));
    ...
    expect(queryByTestId('loading')).toBeNull();
    expect(queryByTestId('data')?.textContent).toBe('{"data":[{"id":1,"title":"foo"}],"total":1}');
  });
  ...
  it('should call getList and show error in development environment', async () => {
    ...
    const getList = jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'foo' }] }));
    const dataProvider = { getList };
    const { queryByTestId } = render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetList />
    ...
    expect(getList).toBeCalledTimes(1);
    expect(queryByTestId('loading')).toBeNull();
    expect(queryByTestId('error')?.textContent).toBe('ra.notification.data_provider_error');
  ...
  it('should call custom and not show error', async () => {
    const getCustom = jest.fn(() => Promise.resolve({ result: [{ id: 1, title: 'foo' }] }));
  ...
  it('should return array or object when 401', async () => {
    ...
    expect(results).toEqual([{ data: [] }, { data: [] }, { data: {} }, { data: [] }]);
  });

  it('should allow prefetching', async () => {
    const getMany = jest.fn().mockResolvedValue({ data: [{ id: 1, name: 'John Doe' }] });
    const dataProvider = {
      getOne: async () => ({
