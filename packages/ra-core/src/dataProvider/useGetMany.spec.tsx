...
describe('useGetMany', () => {
  ...
  it('should not call dataProvider.getMany() on mount if enabled is false', async () => {
    const { rerender } = render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetMany resource="posts" ids={[1]} options={{ enabled: false }} />
      </CoreAdminContext>,
    );
    ...
    rerender(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetMany resource="posts" ids={[1]} options={{ enabled: true }} />
      </CoreAdminContext>,
    );
    await new Promise((resolve) => setTimeout(resolve));
    expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should accept a meta parameter', async () => {
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetMany resource="posts" ids={[1]} meta={{ hello: 'world' }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should execute success side effects on success', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetMany resource="posts" ids={[1]} options={{ onSuccess }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should update loading state when ids change', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseCustomGetMany resource="posts" ids={[1]} callback={hookValue} />
      </CoreAdminContext>,
    );
