...
export const useUpdate = <RecordType extends RaRecord = any, ErrorType = Error>(
  ...
  const dataProviderUpdate = useEvent((resource: string, params: UpdateParams<RecordType>) =>
    ...
    { resource, ...params },
    {
      ...
      updateCache: ({ resource, ...params }, { mutationMode }, result) => {
        ...
        const updateColl = (old: RecordType[]) => {
          ...
          return [...old.slice(0, index), { ...old[index], ...clonedData } as RecordType, ...old.slice(index + 1)];
        };
  ...
  const update = useEvent(
    (
      ...
      callTimeOptions: MutateOptions<RecordType, ErrorType, Partial<UseUpdateMutateParams<RecordType>>, unknown> & {
...
export type UseUpdateOptions<RecordType extends RaRecord = any, ErrorType = Error> = Omit<
  UseMutationOptions<RecordType, ErrorType, Partial<UseUpdateMutateParams<RecordType>>>,
  'mutationFn'
> & {
  mutationMode?: MutationMode;
  returnPromise?: boolean;
  getMutateWithMiddlewares?: <UpdateFunctionType extends DataProvider['update'] = DataProvider['update']>(
...
> = (
  resource?: string,
  params?: Partial<UpdateParams<RecordType>>,
  options?: MutateOptions<RecordType, ErrorType, Partial<UseUpdateMutateParams<RecordType>>, unknown> & {
    mutationMode?: MutationMode;
...
> = [
  ...
  UseMutationResult<RecordType, ErrorType, Partial<UpdateParams<RecordType> & { resource?: string }>, unknown> & {
    isLoading: boolean;
  },
];
