...
import { EditBase, ListBase, RecordsIterator, useRegisterMutationMiddleware } from '../controller';
...
export const MutationMode = ({ timeout = 1000 }) => {
  ...
  return (
    <CoreAdminContext queryClient={new QueryClient()} dataProvider={dataProvider}>
      <MutationModeCore />
    </CoreAdminContext>
  );
};
...
export const Middleware = ({
  middleware = (resource: string, params: any) => {
    console.log(`Updating resource ${resource} with params:`, JSON.stringify(params));
  },
            ...
            <ListBase>
              <ul>
                <RecordsIterator
                  render={(record) => (
                    <li>
                      <LinkBase to={`/posts/${record.id}`}>{record.title}</LinkBase>
                    </li>
                  )}
                />
              </ul>
...
const UpdateMiddleware = ({ middleware }: { middleware: (resource: string, params: any) => void }) => {
...
export const InvalidateList = ({ mutationMode = 'undoable' }: { mutationMode?: MutationModeType }) => {
