...
import { createHeadersFromOptions, queryParameters, flattenObject } from './fetch';
...
describe('queryParameters', () => {
  ...
  it('should generate multiple query parameters', () => {
    ...
    expect(['foo=fooval&bar=barval', 'bar=barval&foo=fooval']).toContain(actual);
  });
  ...
  it('should generate an encoded query parameter', () => {
    ...
    expect(queryParameters({ [data[0]]: data[1] })).toEqual(data.map(encodeURIComponent).join('='));
  });
});
...
describe('createHeadersFromOptions', () => {
  ...
  it('should not add a Content-Type header for GET requests', () => {
    ...
    const headersWithoutMethod = createHeadersFromOptions(optionsWithoutMethod);
  ...
  it('should not add a Content-Type header for DELETE requests with no body', () => {
    ...
    const headersWithDeleteAndBody = createHeadersFromOptions(optionsWithDeleteAndBody);
    expect(headersWithDeleteAndBody.get('Content-Type')).toStrictEqual('application/json');
  });
  ...
  it('should not add a Content-Type header if there already is a Content-Type header', () => {
    ...
    const headersWithContentType = createHeadersFromOptions(optionsWithContentType);
    expect(headersWithContentType.get('Content-Type')).toStrictEqual('not undefined');
  });
});
