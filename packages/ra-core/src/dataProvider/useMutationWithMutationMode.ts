...
>(
  ...
  const getMutateWithMiddlewaresEvent = useEvent(
    getMutateWithMiddlewares ??
      (noop as unknown as (mutate: MutationFunction<TData, TVariables>) => (params: TVariables) => Promise<TData>),
  );
  ...
  const mutateWithMiddlewares = useRef<
    MutationFunction<TData, TVariables> | DataProviderMutationWithMiddlewareFunction<TData, TVariables>
  >(mutationFnEvent);
  ...
  const callTimeOnError = useRef<
    UseMutationWithMutationModeOptions<ErrorType, TData, TVariables>['onError'] | undefined
  >(undefined);
  const callTimeOnSettled = useRef<
    UseMutationWithMutationModeOptions<ErrorType, TData, TVariables>['onSettled'] | undefined
  >(undefined);
  ...
  const mutation = useMutation<TData['data'], ErrorType, Partial<TVariables>>({
    ...
    onError: (...args) => {
      if (mode.current === 'optimistic' || mode.current === 'undoable') {
        ...
        (onMutateResult as { snapshot: Snapshot }).snapshot.forEach(([key, value]) => {
  ...
  const mutate = async (
    ...
    callTimeOptions: MutateOptions<TData['data'], ErrorType, Partial<TVariables>, unknown> & {
    ...
    if (getMutateWithMiddlewares) {
      mutateWithMiddlewares.current = getMutateWithMiddlewaresEvent((params: TVariables) => {
...
type MutationFunction<TData extends { data?: unknown } = { data?: unknown }, TVariables = unknown> = (
...
export type UseMutationWithMutationModeOptions<
  ...
  TData extends { data?: unknown } = { data?: unknown },
  TVariables = unknown,
> = Omit<UseMutationOptions<TData['data'], ErrorType, Partial<TVariables>>, 'mutationFn'> & {
...
> = (
  params?: Partial<TVariables>,
  options?: MutateOptions<TData['data'], ErrorType, Partial<TVariables>, unknown> & {
    mutationMode?: MutationMode;
