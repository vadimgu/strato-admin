...
import type { RaRecord, CreateParams, Identifier, DataProvider, MutationMode, CreateResult } from '../types';
...
>(
  ...
  const dataProviderCreate = useEvent((resource: string, params) =>
    dataProvider.create<RecordType, ResultRecordType>(resource, params as CreateParams<RecordType>),
  );
  ...
  >(
    { resource, ...params },
    {
      ...
      updateCache: ({ resource, ...params }, { mutationMode }, result) => {
        const id = mutationMode === 'pessimistic' ? result?.id : params.data?.id;
        if (!id) {
      ...
      getQueryKeys: ({ resource, ...params }, { mutationMode }) => {
        ...
        if (mutationMode !== 'pessimistic' && params.data?.id) {
          queryKeys.push([resource, 'getOne', { id: String(params.data.id), meta: params.meta }]);
        }
...
> & {
  mutationMode?: MutationMode;
  returnPromise?: boolean;
  getMutateWithMiddlewares?: <CreateFunctionType extends DataProvider['create'] = DataProvider['create']>(
    mutate: CreateFunctionType,
  ) => (...Params: Parameters<CreateFunctionType>) => ReturnType<CreateFunctionType>;
};
...
> = (
  resource?: string,
  params?: Partial<CreateParams<Partial<RecordType>>>,
  options?: MutateOptions<ResultRecordType, MutationError, Partial<UseCreateMutateParams<RecordType>>, unknown> & {
    mutationMode?: MutationMode;
