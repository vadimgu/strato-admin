...
const UseGetList = ({
  ...
  const hookValue = useGetList(resource, { pagination, sort, filter, meta }, options);
...
describe('useGetList', () => {
  ...
  it('should not call the dataProvider on update', async () => {
    const dataProvider = testDataProvider({
      // @ts-ignore
      getList: jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'foo' }], total: 1 })),
    });
  ...
  it('should accept a meta parameter', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetList pagination={{ page: 1, perPage: 20 }} meta={{ hello: 'world' }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getList).toBeCalledWith('posts', {
  ...
  it('should return initial data based on Query Cache', async () => {
    ...
    const dataProvider = testDataProvider({
      // @ts-ignore
      getList: jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'live' }], total: 1 })),
    });
    render(
  ...
  it('should execute success side effects on success', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        ...
        <UseGetList resource="comments" options={{ onSuccess: onSuccess2 }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
