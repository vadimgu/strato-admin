...
import { Middleware, MutationMode, Params, InvalidateList } from './useUpdate.stories';
...
describe('useUpdate', () => {
  ...
  describe('data', () => {
    it('returns a data typed based on the parametric type', async () => {
      ...
      const dataProvider = {
        update: jest.fn(() => Promise.resolve({ data: { id: 1, sku: 'abc' } } as any)),
      } as any;
      let localUpdate;
      let sku;
      const Dummy = () => {
  ...
  describe('query cache', () => {
    it('updates getList query cache when dataProvider promise resolves', async () => {
      ...
      const dataProvider = {
        update: jest.fn(() => Promise.resolve({ data: { id: 1, bar: 'baz' } } as any)),
      } as any;
      let localUpdate;
      const Dummy = () => {
        const [update] = useUpdate();
    ...
    it('updates getManyReference query cache with pageInfo when dataProvider promise resolves', async () => {
      ...
      await waitFor(() => {
        expect(queryClient.getQueryData(['foo', 'getManyReference'])).toEqual({
          data: [{ id: 1, bar: 'baz' }],
          pageInfo: {
            hasPreviousPage: false,
            hasNextPage: true,
    ...
    it('updates getInfiniteList query cache when dataProvider promise resolves', async () => {
      ...
      await waitFor(() => {
        expect(queryClient.getQueryData(['foo', 'getInfiniteList'])).toEqual({
    ...
    describe('pessimistic mutation mode', () => {
      it('updates getOne query cache when dataProvider promise resolves', async () => {
        const queryClient = new QueryClient();
        queryClient.setQueryData(['foo', 'getOne', { id: '1', meta: undefined }], { id: 1, bar: 'bar' });
        const dataProvider = {
          update: jest.fn(() => Promise.resolve({ data: { id: 1, bar: 'baz' } } as any)),
        } as any;
        let localUpdate;
        ...
        await waitFor(() => {
          expect(queryClient.getQueryData(['foo', 'getOne', { id: '1', meta: undefined }])).toEqual({
            id: 1,
            bar: 'baz',
          });
        });
      ...
      it('updates getOne query cache when dataProvider promise resolves with meta', async () => {
        const queryClient = new QueryClient();
        queryClient.setQueryData(['foo', 'getOne', { id: '1', meta: { key: 'value' } }], { id: 1, bar: 'bar' });
        const dataProvider = {
          update: jest.fn(() => Promise.resolve({ data: { id: 1, bar: 'baz' } } as any)),
        } as any;
        let localUpdate;
        ...
        await waitFor(() => {
          expect(queryClient.getQueryData(['foo', 'getOne', { id: '1', meta: { key: 'value' } }])).toEqual({
            id: 1,
            bar: 'baz',
          });
        });
    ...
    describe('optimistic mutation mode', () => {
      it('updates getOne query cache immediately and invalidates query when dataProvider promise resolves', async () => {
        ...
        await waitFor(() => {
          expect(queryClientSpy).toHaveBeenCalledWith({
            queryKey: ['foo', 'getOne', { id: '1', meta: undefined }],
          });
        });
      });
      ...
      it('updates getOne query cache immediately and invalidates query when dataProvider promise resolves with meta', async () => {
        ...
        await waitFor(() => {
          expect(queryClientSpy).toHaveBeenCalledWith({
            queryKey: ['foo', 'getOne', { id: '1', meta: { key: 'value' } }],
          });
        });
      });
  ...
  describe('middlewares', () => {
    it('when pessimistic, it accepts middlewares and displays result and success side effects when dataProvider promise resolves', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByText('success')).toBeNull();
        expect(screen.queryByText('Hello World from middleware')).toBeNull();
        expect(screen.queryByText('mutating')).not.toBeNull();
      });
      await waitFor(() => {
        expect(screen.queryByText('success')).not.toBeNull();
        expect(screen.queryByText('Hello World from middleware')).not.toBeNull();
        expect(screen.queryByText('mutating')).toBeNull();
      });
    });
    ...
    it(`it calls the middlewares in undoable mode even when they got unregistered`, async () => {
      ...
      render(<Middleware mutationMode="undoable" timeout={100} middleware={middlewareSpy} />);
    ...
    it(`it calls the middlewares in optimistic mode even when they got unregistered`, async () => {
      const middlewareSpy = jest.fn();
      render(<Middleware mutationMode="optimistic" timeout={0} middleware={middlewareSpy} />);
