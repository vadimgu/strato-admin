...
import { CreateBase, ListBase, RecordsIterator, useRegisterMutationMiddleware } from '../controller';
...
const MutationModeCore = () => {
  ...
  const [mutationMode, setMutationMode] = React.useState<MutationModeType>('pessimistic');

  const { isPending: isPendingGetOne, data, error, refetch } = useGetOne('posts', { id: 2 });
  ...
  return (
    <>
      ...
      <div>
        ...
        &nbsp;
        <button onClick={() => setMutationMode('optimistic')} disabled={isPending}>
  ...
  return (
    <>
      ...
      <div>
        ...
        &nbsp;
        <button onClick={() => setParams({ title: 'Goodbye World' })} disabled={isPending}>
          Change params
        </button>
        &nbsp;
        <button onClick={() => refetch()}>Refetch</button>
...
export const Middleware = ({
  middleware = (resource: string, params: any) => {
    console.log(`Creating resource ${resource} with params:`, JSON.stringify(params));
  },
      ...
      >
        <Resource
          ...
          list={
            <ListBase>
              <ul>
                <RecordsIterator render={(record) => <li>{record.title}</li>} />
              </ul>
              <RefreshButton />
            </ListBase>
          }
          create={
            <CreateBase
              ...
              redirect="list"
              transform={(data) => ({
                id: mutationMode === 'pessimistic' ? undefined : posts.length + 1,
                ...data,
              })}
            >
              <Form>
...
const CreateMiddleware = ({ middleware }: { middleware: (resource: string, params: any) => void }) => {
...
export const InvalidateList = ({ mutationMode }: { mutationMode: MutationModeType }) => {
        ...
        <Resource
          ...
          create={
            <CreateBase mutationMode={mutationMode}>
              <Form>
                {mutationMode !== 'pessimistic' && <TextInput source="id" defaultValue={3} />}
                <TextInput source="title" />
                <button type="submit">Save</button>
              </Form>
            </CreateBase>
