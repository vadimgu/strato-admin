...
export default { title: '@strato-admin/ra-core/dataProvider/useCreate/optimistic' };
...
const ErrorCore = () => {
  ...
  const { isPending: isPendingGetOne, data, error: getOneError, refetch } = useGetOne('posts', { id: 2 });
  const [create, { isPending }] = useCreate();
  const handleClick = () => {
    create(
      'posts',
