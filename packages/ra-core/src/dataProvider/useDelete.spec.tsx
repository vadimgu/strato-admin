...
import { ErrorCase as ErrorCaseOptimistic, SuccessCase as SuccessCaseOptimistic } from './useDelete.optimistic.stories';
import { ErrorCase as ErrorCaseUndoable, SuccessCase as SuccessCaseUndoable } from './useDelete.undoable.stories';
...
describe('useDelete', () => {
  ...
  it('returns data typed based on the parametric type', async () => {
    ...
    const dataProvider = testDataProvider({
      delete: jest.fn(() => Promise.resolve({ data: { id: 1, sku: 'abc' } } as any)),
    });
    let localDeleteOne;
    let sku;
    const Dummy = () => {
  ...
  describe('mutationMode', () => {
    ...
    it('when pessimistic, forces a refresh of related queries', async () => {
      ...
      // Delete element #2
      const TheLittlePrinceDeleteButton = screen.queryAllByText('Delete')[1];
      ...
      expect(screen.queryByText('The Chronicles of Narnia')).not.toBeNull();
    });
