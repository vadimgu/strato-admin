...
import { EditBase, ListBase, RecordsIterator, useDeleteController, WithRecord } from '../controller';
...
export const MutationMode = ({ dataProvider }: { dataProvider?: DataProvider }) => {
  const posts = [
    { id: 1, title: 'Hello' },
    { id: 2, title: 'World' },
  ];
  ...
  return (
    <CoreAdminContext queryClient={new QueryClient()} dataProvider={dataProvider ?? defaultDataProvider}>
      <MutationModeCore />
    </CoreAdminContext>
  );
};
  ...
  return (
    <>
      ...
      <div>
        ...
        &nbsp;
        <button onClick={() => setParams({ meta: 'test' })} disabled={isPending}>
          Change params
        </button>
        &nbsp;
        <button onClick={() => refetch()}>Refetch</button>
        ...
        <Resource
          ...
          edit={
            <EditBase>
              <div>
                <h1>Edit Post</h1>
                <WithRecord render={(record) => <div>Title: {record.title}</div>} />
