...
import { Middleware, MutationMode, Params, InvalidateList } from './useCreate.stories';
...
describe('useCreate', () => {
  ...
  it('calls onSettled when provided in hook time options', async () => {
    ...
    const Dummy = () => {
      const [create] = useCreate('foo', { data: { bar: 'baz' } }, { onSettled });
      localCreate = create;
      return <span />;
    };
  ...
  it('returns a state typed based on the parametric type', async () => {
    ...
    const dataProvider = testDataProvider({
      create: jest.fn(() => Promise.resolve({ data: { id: 1, sku: 'abc' } } as any)),
    });
    let localCreate;
    let sku;
    const Dummy = () => {
  ...
  describe('middlewares', () => {
    ...
    it(`it calls the middlewares in undoable mode even when they got unregistered`, async () => {
      ...
      render(<Middleware mutationMode="undoable" timeout={0} middleware={middlewareSpy} />);
