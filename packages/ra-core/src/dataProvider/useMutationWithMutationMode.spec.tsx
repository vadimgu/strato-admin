...
import { useMutationWithMutationMode, UseMutationWithMutationModeOptions } from './useMutationWithMutationMode';
...
describe('useMutationWithMutationMode', () => {
  ...
  const useUpdateUserProfile = (
    args?: { data?: any },
    options?: Pick<UseMutationWithMutationModeOptions<Error, { data: any }, { data?: any }>, 'mutationMode'>,
  ) => {
    const dataProvider = useDataProvider<MyDataProvider>();
    return useMutationWithMutationMode<Error, { data: any }, { data?: any }>(args, {
      mutationFn: ({ data }) => {
        if (!data) {
          throw new Error('data is required');
        }
        return dataProvider.updateUserProfile({ data }).then(({ data }) => data);
      },
      updateCache: ({ data }) => {
        return data;
      },
  ...
  it('returns a callback that can be used with update arguments', async () => {
    const dataProvider = testDataProvider({
      updateUserProfile: jest.fn(({ data }) => Promise.resolve({ data: { id: 1, ...data } } as any)),
    }) as MyDataProvider;
    let localUpdate;
    const Dummy = () => {
  ...
  it('uses the latest params at execution time in optimistic mode', async () => {
    ...
    const Dummy = () => {
      ...
      return (
        <>
          <p>{data.value}</p>
          <button onClick={() => setData({ value: 'value2' })}>Update data</button>
