...
import { useGetManyAggregate, type UseGetManyAggregateOptions } from './useGetManyAggregate';
...
describe('useGetManyAggregate', () => {
  ...
  beforeEach(() => {
    dataProvider = testDataProvider({
      getMany: jest.fn().mockResolvedValue({ data: [{ id: 1, title: 'foo' }] }),
    });
  });
  ...
  it('should not call dataProvider.getMany() on mount if enabled is false', async () => {
    const { rerender } = render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} options={{ enabled: false }} />
      </CoreAdminContext>,
    );
    ...
    rerender(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} options={{ enabled: true }} />
      </CoreAdminContext>,
    );
    await new Promise((resolve) => setTimeout(resolve));
    expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should use data from query cache on mount', async () => {
    ...
    rerender(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} callback={hookValue} />
      </CoreAdminContext>,
    );
  ...
  it('should execute success side effects on success', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} options={{ onSuccess }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should execute error side effects on failure', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} options={{ onError }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it('should not aggregate multiple calls for different meta', async () => {
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1, 2]} meta={{ test: true }} />
        <UseGetManyAggregate resource="posts" ids={[3, 4]} meta={{ test: true }} />
        <UseGetManyAggregate resource="posts" ids={[5, 6]} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
  ...
  it('should aggregate multiple calls for the same resource into one even if one of the calls requests all the aggregated ids', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyAggregate resource="posts" ids={[1]} callback={firstCallback} />
        <UseGetManyAggregate resource="posts" ids={[1, 2]} callback={secondCallback} />
        <UseGetManyAggregate resource="posts" ids={[1, 2, 3]} callback={thirdCallback} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
  ...
  it.each([
  ...
  ])('should abort the request if the query is canceled', async ({ queries, expectedQueryKeyParams }) => {
    const abort = jest.fn();
    const dataProvider = testDataProvider({
      getMany: jest.fn(
        (_resource, { signal }) =>
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider} queryClient={queryClient}>
        {queries.map((query, index) => (
          <UseGetManyAggregate key={index} resource="posts" {...query} />
        ))}
      </CoreAdminContext>,
    );
    await waitFor(() => {
    ...
    expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
    expect(dataProvider.getMany).toHaveBeenCalledWith('posts', expect.objectContaining(expectedQueryKeyParams));
  ...
  it('should only call a query that is not yet aborted and then abort it successfully', async () => {
    ...
    const dataProvider = testDataProvider({
      getMany: jest.fn(
        (_resource, { signal }) =>
          new Promise(() => {
            if (signal.aborted) {
              reject('Test failure: called a query which already received an abort signal');
            }
            signal.addEventListener('abort', () => {
              abort(signal.reason);
            });
    ...
    expect(dataProvider.getMany).toHaveBeenCalledTimes(1);
    expect(dataProvider.getMany).toHaveBeenCalledWith('posts', expect.objectContaining({ ids: ['1'] }));
