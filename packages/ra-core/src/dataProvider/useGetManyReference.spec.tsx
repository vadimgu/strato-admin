...
const UseGetManyReference = ({
  ...
  const hookValue = useGetManyReference(resource, { target, id, pagination, sort, filter, meta }, options);
...
describe('useGetManyReference', () => {
  ...
  it('should not call the dataProvider on update', async () => {
    const dataProvider = testDataProvider({
      // @ts-ignore
      getManyReference: jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'foo' }], total: 1 })),
    });
  ...
  it('should return initial data based on Query Cache', async () => {
    ...
    const dataProvider = testDataProvider({
      // @ts-ignore
      getManyReference: jest.fn(() => Promise.resolve({ data: [{ id: 1, title: 'live' }], total: 1 })),
    });
    render(
    ...
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ data: [{ id: 1, title: 'cached' }] }));
    });
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ data: [{ id: 1, title: 'live' }] }));
    });
  });
  ...
  it('should return isFetching false once the dataProvider returns', async () => {
    ...
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ isFetching: true }));
    });
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ isFetching: false }));
    });
  });
  ...
  it('should set the error state when the dataProvider fails', async () => {
    ...
    const dataProvider = testDataProvider({
      // @ts-ignore
      getManyReference: jest.fn(() => Promise.reject(new Error('failed'))),
    });
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <UseGetManyReference options={{ retry: false }} callback={callback} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ error: null }));
    });
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ error: new Error('failed') }));
    });
  });
  ...
  it('should execute success side effects on success', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        ...
        <UseGetManyReference resource="comments" options={{ onSuccess: onSuccess2 }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
  ...
  it('should pre-populate getOne Query Cache', async () => {
    ...
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toEqual({ id: 1, title: 'live' });
  });
  ...
  it('should still pre-populate getOne Query Cache with custom onSuccess', async () => {
    ...
    render(
      <CoreAdminContext queryClient={queryClient} dataProvider={dataProvider}>
        <UseGetManyReference callback={callback} options={{ onSuccess }} />
      </CoreAdminContext>,
    );
    await waitFor(() => {
      expect(callback).toHaveBeenCalledWith(expect.objectContaining({ data: [{ id: 1, title: 'live' }] }));
    ...
    await waitFor(() => {
      expect(onSuccess).toHaveBeenCalledWith(expect.objectContaining({ data: [{ id: 1, title: 'live' }] }));
    });
    expect(queryClient.getQueryData(['posts', 'getOne', { id: '1' }])).toEqual({ id: 1, title: 'live' });
  });
