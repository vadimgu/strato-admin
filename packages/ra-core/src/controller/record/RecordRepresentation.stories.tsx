...
import { ResourceContextProvider, ResourceDefinitionContextProvider } from '../../core';
    ...
    <ResourceDefinitionContextProvider
      definitions={{
        books: {
          ...
          hasShow: true,
          hasCreate: true,
          recordRepresentation: (record) => `${record.title} by ${record.author}`,
        },
      }}
    >
      <RecordContextProvider value={Book}>
...
const BookRepresentation = () => {
  ...
  return (
    <p>
      <b>{record.title}</b>{' '}
      <i>
        (by {record.author}) - {new Date(record.publishedAt).getFullYear()}
      </i>
    </p>
  );
};
