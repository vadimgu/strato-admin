...
const useSortState = (initialSort: SortPayload = defaultSort): SortProps => {
  ...
  return {
    setSort: useCallback((sort: SortPayload) => dispatch({ type: 'SET_SORT', payload: sort }), [dispatch]),
    setSortField: useCallback((field: string) => dispatch({ type: 'SET_SORT_FIELD', payload: field }), [dispatch]),
