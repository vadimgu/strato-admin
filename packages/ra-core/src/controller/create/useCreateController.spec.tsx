...
import { Middleware, SaveContextProvider, useRegisterMutationMiddleware } from '../saveContext';
import { CreateController } from './CreateController';

import { TestMemoryRouter } from '../../routing';
import { CanAccess, DisableAuthentication } from './useCreateController.security.stories';
...
describe('useCreateController', () => {
  ...
  it('should call the dataProvider.create() function on save', async () => {
    const dataProvider = testDataProvider({
      ...
      create: jest.fn().mockImplementationOnce((_, { data }) => Promise.resolve({ data: { id: 123, ...data } })),
    });
    let saveCallback;
    render(
  ...
  it('should allow mutationOptions to override the default success side effects', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Notification />
        <CreateController {...defaultProps} mutationOptions={{ onSuccess }}>
          {({ save }) => {
            saveCallback = save;
            return null;
          }}
  ...
  it('should allow mutationOptions to override the default failure side effects', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Notification />
        <CreateController {...defaultProps} mutationOptions={{ onError }}>
          {({ save }) => {
            saveCallback = save;
            return null;
          }}
  ...
  it('should accept meta in mutationOptions', async () => {
    let saveCallback;
    const create = jest.fn().mockImplementationOnce((_, { data }) => Promise.resolve({ data: { id: 123, ...data } }));
    const dataProvider = testDataProvider({
      getOne: () => Promise.resolve({ data: { id: 12 } } as any),
      create,
    });
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <CreateController {...defaultProps} mutationOptions={{ meta: { lorem: 'ipsum' } }}>
          {({ save }) => {
            saveCallback = save;
            return null;
          }}
  ...
  it('should allow to register middlewares', async () => {
    ...
    const middleware: Middleware<DataProvider['create']> = jest.fn((resource, params, next) => {
