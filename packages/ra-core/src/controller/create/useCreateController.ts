...
import { HttpError, useCreate, UseCreateMutateParams } from '../../dataProvider';
...
>(
  ...
  const [create, { isPending: saving }] = useCreate<RecordType, MutationOptionsError, ResultRecordType>(
    resource,
    undefined,
    {
  ...
  const save = useCallback(
    (
      ...
      Promise.resolve(transformFromSave ? transformFromSave(data) : transform ? transform(data) : data).then(
        async (data: Partial<RecordType>) => {
          try {
            await create(
              resource,
            ...
            if (
              (error instanceof HttpError ||
                (typeof error === 'object' && error !== null && error.hasOwnProperty('body'))) &&
