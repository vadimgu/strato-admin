...
import { Basic, defaultDataProvider } from './useReferenceManyFieldController.stories';
...
describe('useReferenceManyFieldController', () => {
  ...
  it('should store selection state linked to referencing record', async () => {
    ...
    render(
      <CoreAdminContext store={store}>
        ...
        >
          {({ onToggleItem }) => {
            return <button onClick={() => onToggleItem(456)}>Toggle</button>;
          }}
        </ReferenceManyFieldController>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(setStore).toHaveBeenCalledWith('authors.123.books.selectedIds', [456]);
    });
  });
  ...
  it('should support custom storeKey', async () => {
    ...
    fireEvent.click(await screen.findByText('Toggle'));
    await waitFor(() => {
      expect(setStore).toHaveBeenCalledWith('customKey.selectedIds', [456]);
    });
  });
  ...
  describe('response metadata', () => {
    it('should return response metadata as meta', async () => {
      ...
      const ListMetadataInspector = () => {
        ...
        return (
          <>
            Response metadata: <pre>{JSON.stringify(listContext.meta, null)}</pre>
          </>
        );
      };
      ...
      await screen.findByText('{"facets":[{"foo":"bar","count":1},{"foo":"baz","count":2}]}');
    });
  });
  ...
  describe('onSelectAll', () => {
    it('should select all records', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [0,1]');
      });
    });
    ...
    it('should not select more records than the provided limit', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [0]');
      });
      await waitFor(() => {
