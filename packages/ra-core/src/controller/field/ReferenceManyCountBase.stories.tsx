...
import { QueryClientProvider, QueryClient, onlineManager } from '@tanstack/react-query';
    ...
    <DataProviderContext.Provider value={dataProvider}>
      ...
      >
        <ResourceContextProvider value="posts">
          <RecordContextProvider value={post}>{children}</RecordContextProvider>
...
export const LoadingState = () => (
  <Wrapper dataProvider={{ getManyReference: () => new Promise(() => {}) }}>
    <ReferenceManyCountBase reference="comments" target="post_id" loading="loading..." />
  </Wrapper>
);
...
export const ErrorState = () => (
  ...
  >
    <ReferenceManyCountBase reference="comments" target="post_id" error="Error!" />
  </Wrapper>
);
      ...
      getManyReference: (resource, params) =>
        Promise.resolve({
          data: comments
            ...
            .filter((post) => Object.keys(params.filter).every((key) => post[key] === params.filter[key])),
          total: comments
            .filter((c) => c.post_id === 1)
            .filter((post) => Object.keys(params.filter).every((key) => post[key] === params.filter[key])).length,
        }),
    }}
  >
      ...
      <div>
        <RenderChildOnDemand>
          <ReferenceManyCountBase
            ...
            target="post_id"
            loading="Loading..."
            offline={<span style={{ color: 'orange' }}>You are offline, cannot load data</span>}
          />
        </RenderChildOnDemand>
      </div>
