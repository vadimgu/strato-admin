...
import { Basic, Errored, Loading, Meta, Offline, WithRenderProp, ZeroIndex } from './ReferenceFieldBase.stories';
...
describe('<ReferenceFieldBase />', () => {
  ...
  it('should accept meta in queryOptions', async () => {
    const getMany = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: [], total: 25 }));
    const dataProvider = testDataProvider({
      getMany,
      getOne: () =>
        // @ts-ignore
  ...
  describe('with render prop', () => {
    it('should display an error if error is defined', async () => {
      ...
      render(<WithRenderProp dataProvider={dataProviderWithAuthorsError} />);
      await waitFor(() => {
    ...
    it('should pass the loading state', async () => {
      ...
      render(<WithRenderProp dataProvider={dataProviderWithAuthorsLoading} />);
      await waitFor(() => {
        expect(screen.queryByText('Loading...')).not.toBeNull();
      });
    });
