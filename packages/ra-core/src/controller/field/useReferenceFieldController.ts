...
export const useReferenceFieldController = <ReferenceRecordType extends RaRecord = RaRecord, ErrorType = Error>(
  ...
  const referenceRecordQuery = useReference<ReferenceRecordType, ErrorType>({
    ...
    options: {
      ...queryOptions,
      enabled: (queryOptions?.enabled == null || queryOptions?.enabled === true) && id != null,
    },
  });
