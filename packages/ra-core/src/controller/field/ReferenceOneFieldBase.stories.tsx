...
export default { title: '@strato-admin/ra-core/controller/field/ReferenceOneFieldBase' };
  ...
  <TestMemoryRouter initialEntries={['/books/1/show']}>
    <CoreAdminContext dataProvider={dataProvider}>
      <ResourceContextProvider value="books">
        <RecordContextProvider value={{ id: 1, title: 'War and Peace' }}>{children}</RecordContextProvider>
      </ResourceContextProvider>
    </CoreAdminContext>
  </TestMemoryRouter>
);
...
export const WithRenderProp = ({ dataProvider = defaultDataProvider }: { dataProvider?: any }) => {
  return (
    <Wrapper dataProvider={dataProvider}>
      <ReferenceOneFieldBase
        ...
        render={({ isPending, error, referenceRecord }) => {
          ...
          return <span>{referenceRecord ? referenceRecord.ISBN : ''}</span>;
        }}
      />
    </Wrapper>
  );
