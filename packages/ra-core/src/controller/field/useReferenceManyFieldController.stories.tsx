...
const defaultRenderProp = (props: ListControllerResult) => (
  <div>
    ...
    >
      <button onClick={() => props.onSelectAll()} disabled={props.total === props.selectedIds.length}>
        Select All
      </button>
      <button onClick={() => props.onSelectAll({ limit: 1 })} disabled={props.selectedIds.length >= 1}>
        Limited Select All
      </button>
      <button onClick={props.onUnselectItems} disabled={props.selectedIds.length === 0}>
        Unselect All
      </button>
      <p data-testid="selected_ids">Selected ids: {JSON.stringify(props.selectedIds)}</p>
    </div>
    <ul
      style={{
        listStyleType: 'none',
...
export default {
  title: '@strato-admin/ra-core/controller/field/useReferenceManyFieldController',
