...
import type { FilterPayload, GetListResult, RaRecord, SortPayload } from '../../types';
...
export const useReferenceInputController = <RecordType extends RaRecord = any>(
  ...
  const {
    ...
    },
    {
      ...
      ...(otherQueryOptions as UseQueryOptions<GetListResult<RecordType>>),
    },
  );
  ...
  const [referenceRecord, setReferenceRecord] = useState<RecordType | undefined>(undefined);
  useEffect(() => {
  ...
  const { finalData, finalTotal } = useMemo(() => {
    ...
    if (
      ...
      if (referenceRecord && finalData.find((r) => r.id === referenceRecord.id) == null) {
  ...
  return useMemo(
    () =>
      ({
        ...
        isPending,
        isPlaceholderData: isPlaceholderDataReference || isPlaceholderDataPossibleValues,
        page: params.page,
        perPage: params.perPage,
        refetch,
        resource: reference,
