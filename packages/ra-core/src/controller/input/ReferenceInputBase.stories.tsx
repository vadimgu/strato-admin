...
import { ChoicesProps, Form, InputProps, useChoices, useChoicesContext, useInput } from '../../form';
...
const AutocompleteInput = (
  props: Omit<InputProps, 'source'> & Partial<Pick<InputProps, 'source'>> & ChoicesProps & { source?: string },
) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
      <label htmlFor={`${source}-search`}>{props.label || field.name}</label>
      ...
      <input
        ...
        list={`${source}-choices`}
        onChange={(e) => {
          const choice = allChoices?.find((choice) => getChoiceText(choice).toString() === e.target.value);
          if (choice) {
      ...
      <datalist id={`${source}-choices`}>
        {allChoices?.map((choice) => (
          <option key={getChoiceValue(choice)} value={getChoiceText(choice).toString()}>
...
export const Loading = () => (
  <CoreAdminContext dataProvider={dataProvider} i18nProvider={i18nProvider}>
    <Form onSubmit={() => {}} defaultValues={{ tag_ids: [5] }}>
      ...
      >
        <ReferenceInputBase reference="tags" resource="posts" source="tag_ids">
    ...
    >
      <Form>
        ...
        <ReferenceInputBase reference="books" source="self_reference">
          <SelectInput optionText="last_name" label="Self reference" />
        </ReferenceInputBase>
        <button type="submit">Save</button>
      </Form>
    </EditBase>
...
const BookEditQueryOptions = () => {
  ...
  return (
    <EditBase mutationMode="pessimistic">
      <button onClick={() => setEnabled(!enabled)}>Toggle queryOptions</button>
      <Form>
        <TextInput source="title" />
        <ReferenceInputBase reference="authors" source="author" queryOptions={{ enabled }}>
          <SelectInput optionText="last_name" />
        </ReferenceInputBase>
        <button type="submit">Save</button>
      </Form>
...
const BookEditMeta = () => {
  return (
    <EditBase mutationMode="pessimistic">
      <Form>
        <TextInput source="title" />
        <ReferenceInputBase reference="authors" source="author" queryOptions={{ meta: { test: true } }}>
          <SelectInput optionText="last_name" />
        </ReferenceInputBase>
        <button type="submit">Save</button>
      </Form>
