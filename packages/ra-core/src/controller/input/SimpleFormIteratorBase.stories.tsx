...
import { SimpleFormIteratorBase, SimpleFormIteratorBaseProps } from './SimpleFormIteratorBase';
...
const SimpleFormIterator = ({ children, ...props }: SimpleFormIteratorBaseProps) => {
    ...
    <SimpleFormIteratorBase getItemDefaults={getItemDefaults} {...props}>
      <ul>
        {fields.map((member, index) => (
          <SimpleFormIteratorItemBase key={member.id} index={index} record={records[index]}>
            <li>
              {children}
              <RemoveItemButton />
            </li>
        ...
        list={
          <ListBase>
            <DataTable>
              ...
              <DataTable.Col
                label="Tags"
                render={(record) => (record.tags ? record.tags.map((tag: any) => tag.name).join(', ') : '')}
              />
            </DataTable>
          </ListBase>
        }
