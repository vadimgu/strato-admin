...
export const getStatusForInput = <RecordType extends RaRecord = RaRecord>({
  ...
  return {
    ...
    error:
      (field.value && selectedReferenceError && matchingReferencesError) || (!field.value && matchingReferencesError)
        ? field.value
...
export const getStatusForArrayInput = <RecordType extends RaRecord = any>({
  ...
  const choices = Array.isArray(matchingReferences)
    ? referenceRecords.concat(
        matchingReferences.filter((choice) => referenceRecords.findIndex((c) => c.id === choice.id) === -1),
      )
    : referenceRecords;

  return {
    waiting:
      (!matchingReferences && field.value && selectedReferencesDataStatus === REFERENCES_STATUS_EMPTY) ||
      (!matchingReferences && !field.value),
    error:
      matchingReferencesError &&
      (!field.value || (field.value && selectedReferencesDataStatus === REFERENCES_STATUS_EMPTY))
        ? translate('ra.input.references.all_missing', {
            _: 'ra.input.references.all_missing',
          })
        : null,
    warning:
      matchingReferencesError || (field.value && selectedReferencesDataStatus !== REFERENCES_STATUS_READY)
        ? matchingReferencesError ||
