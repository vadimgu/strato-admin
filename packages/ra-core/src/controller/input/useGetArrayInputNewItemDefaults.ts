...
export const useGetArrayInputNewItemDefaults = (fields: ArrayInputContextValue['fields']) => {
  ...
  return useEvent((inputs?: ReactNode) => {
    ...
    Children.forEach(inputs, (input) => {
      if (isValidElement(input) && input.type !== FormDataConsumer && (input.props as any).source) {
        defaultValue[(input.props as any).source] = (input.props as any).defaultValue ?? null;
      }
    });
    return defaultValue;
  });
