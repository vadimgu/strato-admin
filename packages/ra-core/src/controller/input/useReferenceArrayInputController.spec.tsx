...
import { useReferenceArrayInputController, UseReferenceArrayInputParams } from './useReferenceArrayInputController';
...
describe('useReferenceArrayInputController', () => {
  ...
  describe('isLoading', () => {
    it('should set isLoading to true as long as there are no references fetched and no selected references', () => {
      const children = jest.fn(({ isLoading }) => <div>{isLoading.toString()}</div>);
      render(
        <CoreAdminContext
        ...
        >
          <Form defaultValues={{ tag_ids: [1, 2] }} onSubmit={jest.fn()}>
            <ReferenceArrayInputController {...defaultProps}>{children}</ReferenceArrayInputController>
          </Form>
        </CoreAdminContext>,
      );
    ...
    it('should set isLoading to false once the dataProvider returns', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Form onSubmit={jest.fn()} defaultValues={{ tag_ids: [1, 2] }}>
            <ReferenceArrayInputController {...defaultProps}>{children}</ReferenceArrayInputController>
          </Form>
        </CoreAdminContext>,
      );
  ...
  describe('error', () => {
    it('should set error in case of references fetch error and there are no selected reference in the input value', async () => {
      ...
      const children = jest.fn(({ error }) => <div>{error?.message}</div>);
      render(
        <CoreAdminContext
          dataProvider={testDataProvider({
    ...
    it('should set error in case of references fetch error and there are no data found for the references already selected', async () => {
      ...
      render(
        ...
        >
          <Form onSubmit={jest.fn()}>
            <ReferenceArrayInputController {...defaultProps} field={{ value: [1] }}>
              {children}
    ...
    it.skip('should not display an error in case of references fetch error but data from at least one selected reference was found', async () => {
      ...
      render(
        ...
        >
          <Form onSubmit={jest.fn()}>
            <ReferenceArrayInputController {...defaultProps} field={{ value: [1, 2] }}>
              {children}
            </ReferenceArrayInputController>
          </Form>
        </CoreAdminContext>,
  ...
  it('should call getList on mount with default params', async () => {
    ...
    const dataProvider = testDataProvider({
      // @ts-ignore
      getList: jest.fn().mockResolvedValue(Promise.resolve({ data: [], total: 0 })),
    });
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form onSubmit={jest.fn()}>
  ...
  it('should call getList with meta when provided', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form onSubmit={jest.fn()}>
          <ReferenceArrayInputController {...defaultProps} queryOptions={{ meta: { value: 'a' } }}>
            {children}
          </ReferenceArrayInputController>
        </Form>
      </CoreAdminContext>,
  ...
  it('should call getList when setFilters is called', async () => {
    const children = jest.fn(({ setFilters }) => (
      <button aria-label="Filter" onClick={() => setFilters({ q: 'bar' })} />
    ));
    const dataProvider = testDataProvider({
      getList: jest.fn().mockResolvedValue(Promise.resolve({ data: [], total: 0 })),
    });
  ...
  it('should call getMany on mount if value is set', async () => {
    ...
    const dataProvider = testDataProvider({
      // @ts-ignore
      getMany: jest.fn().mockResolvedValue(Promise.resolve({ data: [{ id: 5 }, { id: 6 }] })),
    });
    render(
      <CoreAdminContext dataProvider={dataProvider}>
  ...
  it('should not call getMany when calling setFilters', async () => {
    ...
    const dataProvider = testDataProvider({
      ...
      getList: jest.fn().mockResolvedValue(Promise.resolve({ data: [], total: 0 })),
      // @ts-ignore
      getMany: jest.fn().mockResolvedValue(Promise.resolve({ data: [{ id: 5 }] })),
    });
    render(
      <CoreAdminContext dataProvider={dataProvider}>
  ...
  it('should not call getMany when props other than input are changed from outside', async () => {
    ...
    rerender(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form onSubmit={onSubmit} record={record}>
          <ReferenceArrayInputController {...defaultProps} filter={{ permanentFilter: 'bar' }}>
            {children}
          </ReferenceArrayInputController>
        </Form>
      </CoreAdminContext>,
  ...
  it('should have props compatible with the ListContext', async () => {
    const children = ({ setPage, setPerPage, setSort }) => {
      ...
      return (
        <>
          ...
          <button aria-label="setPerPage" onClick={handleSetPerPage} />
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form onSubmit={jest.fn()}>
          <ReferenceArrayInputController {...defaultProps} field={{ value: [5, 6] }}>
            {children}
          </ReferenceArrayInputController>
        </Form>
      </CoreAdminContext>,
  ...
  it('should call its children with the correct resource', () => {
    ...
    expect(children).toHaveBeenCalledWith(expect.objectContaining({ resource: 'tags' }));
  });
  ...
  describe('enableGetChoices', () => {
    it('should not fetch possible values using getList on load but only when enableGetChoices returns true', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Form onSubmit={jest.fn()}>
            <ReferenceArrayInputController {...defaultProps} enableGetChoices={enableGetChoices}>
              {children}
            </ReferenceArrayInputController>
          </Form>
        </CoreAdminContext>,
    ...
    it('should fetch current value using getMany even if enableGetChoices is returning false', async () => {
      ...
      const dataProvider = testDataProvider({
        ...
        getList: jest.fn().mockResolvedValue(Promise.resolve({ data: [], total: 0 })),
        // @ts-ignore
        getMany: jest.fn().mockResolvedValue({ data: [{ id: 5 }, { id: 6 }] }),
      });
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Form onSubmit={jest.fn()} record={{ tag_ids: [5, 6] }}>
            <ReferenceArrayInputController {...defaultProps} enableGetChoices={() => false}>
              {children}
            </ReferenceArrayInputController>
          </Form>
        </CoreAdminContext>,
    ...
    it('should set isLoading to false if enableGetChoices returns false', async () => {
      ...
      await waitFor(() => {
        expect(children).toHaveBeenCalledWith(expect.objectContaining({ isLoading: false }));
      });
    });
  });
});
