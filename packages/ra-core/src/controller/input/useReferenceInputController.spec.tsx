...
import { useReferenceInputController, UseReferenceInputControllerParams } from './useReferenceInputController';
...
describe('useReferenceInputController', () => {
  ...
  it('should fetch possible values using getList', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form>
          <ReferenceInputController {...defaultProps}>{children}</ReferenceInputController>
        </Form>
      </CoreAdminContext>,
    );
  ...
  it('should allow getList pagination and sorting customization', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form>
          <ReferenceInputController {...defaultProps} page={5} perPage={10} sort={{ field: 'title', order: 'ASC' }}>
            {children}
  ...
  it('should pass possibleValues and record to child', async () => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form defaultValues={{ post_id: 1 }}>
          <ReferenceInputController {...defaultProps} sort={{ field: 'title', order: 'ASC' }}>
            {children}
          </ReferenceInputController>
        </Form>
      </CoreAdminContext>,
  ...
  it('should refetch reference getList when its props change', async () => {
    ...
    const Component = () => {
      ...
      const handleClick = useCallback(() => setSort({ field: 'body', order: 'DESC' }), [setSort]);
      return (
        <>
  ...
  describe('enableGetChoices', () => {
    it('should not fetch possible values using getList on load but only when enableGetChoices returns true', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Form defaultValues={{ post_id: 1 }}>
            <ReferenceInputController {...defaultProps} enableGetChoices={enableGetChoices}>
              {children}
            </ReferenceInputController>
          </Form>
        </CoreAdminContext>,
    ...
    it('should fetch current value using getMany even if enableGetChoices is returning false', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Form defaultValues={{ post_id: 1 }}>
            <ReferenceInputController {...defaultProps} enableGetChoices={() => false}>
              {children}
            </ReferenceInputController>
          </Form>
        </CoreAdminContext>,
