...
import { SelfReference, QueryOptions, Meta, Offline } from './ReferenceInputBase.stories';
...
describe('<ReferenceInputBase />', () => {
  ...
  it('should use meta when fetching current value', async () => {
    ...
    const getMany = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: [] }));
    const dataProvider = testDataProvider({ getList, getMany });
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <Form record={{ post_id: 23 }}>
          <ReferenceInputBase {...defaultProps} queryOptions={{ meta: { foo: 'bar' } }}>
            <AutocompleteInput />
          </ReferenceInputBase>
        </Form>
      </CoreAdminContext>,
