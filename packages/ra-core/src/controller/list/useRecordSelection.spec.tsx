...
describe('useRecordSelection', () => {
  const wrapper = ({ children }) => <StoreContextProvider value={memoryStore()}>{children}</StoreContextProvider>;

  it('should return empty array by default', () => {
    const { result } = renderHook(() => useRecordSelection({ resource: 'foo' }), {
      wrapper,
    });
  ...
  describe('using storeKey', () => {
    ...
    it('should use the stored value', () => {
      const { result } = renderHook(() => useRecordSelection({ resource: 'foo', storeKey: 'bar' }), {
        wrapper: ({ children }) => (
          <StoreContextProvider value={memoryStore()}>
            <StoreSetter name="bar.selectedIds" value={[123, 456]}>
              {children}
            </StoreSetter>
          </StoreContextProvider>
        ),
    ...
    describe('using stored storeKeys', () => {
      it('should keep final storeKey in the store', async () => {
        ...
        renderHook(
          ...
          {
            wrapper: ({ children }) => <StoreContextProvider value={store}>{children}</StoreContextProvider>,
          },
        );

        await waitFor(() => {
          const storeKeys = store.getItem('foo.selectedIds.storeKeys');
      ...
      it('should check all storeKeys listed in store when `fromAllStoreKeys` is `true`', async () => {
        ...
        const { result } = renderHook(
          ...
          },
          {
            wrapper: ({ children }) => (
              <StoreContextProvider value={store}>
                <StoreSetter name={'foo.selectedIds.storeKeys'} value={['bar2.selectedIds']}>
                  <StoreSetter name={'bar1.selectedIds'} value={[123]}>
                    <StoreSetter name={'bar2.selectedIds'} value={[123]}>
                      {children}
                    </StoreSetter>
                  </StoreSetter>
                </StoreSetter>
