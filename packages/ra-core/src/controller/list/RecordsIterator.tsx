...
export const RecordsIterator = <RecordType extends RaRecord = any>(props: RecordsIteratorProps<RecordType>) => {
  const { children, render } = props;
  const { data, total, isPending, error } = useListContextWithProps<RecordType>(props);

  if (isPending === true || error || data == null || data.length === 0 || total === 0) {
    return null;
  }
  ...
  return (
    <>
      {data.map((record, index) => (
        <RecordContextProvider key={record.id ?? `row${index}`} value={record}>
...
export interface RecordsIteratorProps<RecordType extends RaRecord = any> extends Partial<
