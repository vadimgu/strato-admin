...
import { ListsUsingSameResource, ListsWithoutStore } from './useListController.storeKey.stories';
...
describe('useListController', () => {
  describe('storeKey', () => {
    it('should keep distinct two lists of the same resource given different keys', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText('perPage').getAttribute('data-value')).toEqual('3');
      });
      ...
      await waitFor(() => {
        expect(screen.getByLabelText('perPage').getAttribute('data-value')).toEqual('4');
      });
    ...
    it('should not use the store when storeKey is false', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByLabelText('perPage').getAttribute('data-value')).toEqual('5');
      });
