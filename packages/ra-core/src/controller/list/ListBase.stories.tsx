...
const defaultDataProvider = (delay = 300) => fakeRestProvider(data, process.env.NODE_ENV !== 'test', delay);

const BookListView = () => {
  const { data, error, isPending, sort, filterValues, page, perPage, setPage, setPerPage, setFilters, setSort } =
    useListContext();
  ...
  const handleClick = () => {
    ...
    if (JSON.stringify(value.filterValues) !== JSON.stringify(filterValues)) {
    ...
    if (value.sort.field !== sort.field || value.sort.order !== sort.order) {
      setSort(value.sort);
    }
  };
...
export const NoAuthProvider = ({ dataProvider = defaultDataProvider() }: { dataProvider?: DataProvider }) => (
...
export const WithAuthProviderNoAccessControl = ({
  ...
  <CoreAdminContext authProvider={authProvider} dataProvider={dataProvider}>
    <ListBase resource="books" perPage={5} authLoading={<div>Authentication loading...</div>} {...ListProps}>
      <BookListView />
    </ListBase>
  </CoreAdminContext>
);
...
export const AccessControl = ({
  ...
  <CoreAdminContext authProvider={authProvider} dataProvider={dataProvider}>
    <ListBase resource="books" perPage={5} authLoading={<div>Authentication loading...</div>}>
      <BookListView />
    </ListBase>
  </CoreAdminContext>
);
...
const ListMetadataInspector = () => {
  ...
  return (
    <>
      Response metadata: <pre>{JSON.stringify(listContext.meta, null, 2)}</pre>
    </>
  );
};
...
export const DefaultTitle = ({
  ...
  i18nProvider = translations === 'default' ? defaultI18nProvider : customI18nProvider,
}: {
  i18nProvider?: I18nProvider;
  translations?: 'default' | 'resource specific';
}) => (
  <CoreAdminContext dataProvider={defaultDataProvider()} i18nProvider={i18nProvider}>
    <ListBase resource="books" perPage={5}>
      <Title />
    </ListBase>
  </CoreAdminContext>
...
export const WithRenderProps = ({ dataProvider = defaultDataProvider() }: { dataProvider?: DataProvider }) => (
  <CoreAdminContext dataProvider={dataProvider}>
    <ListBase
      resource="books"
      perPage={5}
      render={(controllerProps) => {
        const { data, error, isPending, page, perPage, setPage, total } = controllerProps;
        if (isPending) {
          return <div>Loading...</div>;
        }
        if (error) {
        ...
        return (
          <div>
            ...
            <button disabled={page >= total / perPage} onClick={() => setPage(page + 1)}>
              next
            </button>
            <ul>
              {data.map((record: any) => (
        ...
        render={(controllerProps) => {
          ...
          return (
            <div>
              ...
              <IsOffline>
                <p style={{ color: 'orange' }}>You are offline, the data may be outdated</p>
              </IsOffline>
              <button disabled={page <= 1} onClick={() => setPage(page - 1)}>
                previous
              </button>
...
export const Loading = () => {
  ...
  const dataProvider = {
    ...
    getList: (resource, params) => {
      return new Promise<GetListResult>((resolve) => {
        resolveGetList = () => resolve(baseProvider.getList(resource, params));
      });
    },
  };
  ...
  return (
    <CoreAdminContext dataProvider={dataProvider}>
      ...
      <ListBase resource="books" perPage={5} loading={<p>Loading books...</p>}>
        <BookListView />
      </ListBase>
    </CoreAdminContext>
  );
...
export const FetchError = () => {
  ...
  return (
    <CoreAdminContext dataProvider={dataProvider}>
      ...
      <ListBase resource="books" perPage={5} error={<p>Cannot load books.</p>}>
        <BookListView />
      </ListBase>
    </CoreAdminContext>
  );
...
export const Empty = () => {
  ...
  return (
    <CoreAdminContext dataProvider={dataProvider}>
      ...
      <ListBase resource="books" perPage={5} loading={<p>Loading...</p>} empty={<p>No books</p>}>
        <BookListView />
      </ListBase>
    </CoreAdminContext>
  );
