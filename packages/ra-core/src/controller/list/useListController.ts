...
import { useDataProvider, useGetList, UseGetListHookValue, UseGetListOptions } from '../../dataProvider';
 ...
 * }
 */
export const useListController = <RecordType extends RaRecord = any, ErrorType = Error>(
  ...
  const getData = useEvent(async ({ maxResults, meta: metaOverride }: GetDataOptions = {}) => {
    ...
    const { data } = await dataProvider.getList(resource, {
      sort: currentSort,
      filter: filter ? { ...query.filterValues, ...filter } : query.filterValues,
...
export type ListControllerResult<RecordType extends RaRecord = any, ErrorType = Error> =
