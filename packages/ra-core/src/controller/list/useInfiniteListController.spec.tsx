...
describe('useInfiniteListController', () => {
  ...
  describe('onSelectAll', () => {
    it('should select all records', async () => {
      render(<Basic />);
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: []');
      });
      fireEvent.click(screen.getByText('Select All'));
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [1,2,3,4,5,6,7]');
      });
    });
    it('should select all records even though some records are already selected', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [1]');
      });
      fireEvent.click(screen.getByText('Select All'));
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [1,2,3,4,5,6,7]');
    ...
    it('should not select more records than the provided limit', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('selected_ids').textContent).toBe('Selected ids: [1,2,3]');
      });
      await waitFor(() => {
  ...
  describe('queryOptions', () => {
    it('should accept custom client query options', async () => {
      ...
      const getList = jest.fn().mockImplementationOnce(() => Promise.reject(new Error()));
      const onError = jest.fn();
      const dataProvider = testDataProvider({ getList });
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <InfiniteListController resource="posts" queryOptions={{ onError }}>
            {() => <div />}
    ...
    it('should accept meta in queryOptions', async () => {
      const getList = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: [], total: 25 }));
      const dataProvider = testDataProvider({ getList });
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <InfiniteListController resource="posts" queryOptions={{ meta: { foo: 'bar' } }}>
            {() => <div />}
          </InfiniteListController>
        </CoreAdminContext>,
      );
    ...
    it('should reset page when enabled is set to false', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <InfiniteListController disableSyncWithLocation queryOptions={{ enabled: false }} {...props} />
        </CoreAdminContext>,
      );
  ...
  describe('setFilters', () => {
    ...
    it('should update data if permanent filters change', () => {
      ...
      const getList = jest.fn().mockImplementation(() => Promise.resolve({ data: [], total: 0 }));
      const dataProvider = testDataProvider({ getList });
      ...
      expect(getList).toHaveBeenCalledWith('posts', expect.objectContaining({ filter: { foo: 1 } }));
      ...
      expect(getList).toHaveBeenCalledWith('posts', expect.objectContaining({ filter: { foo: 2 } }));
          ...
          <InfiniteListController {...defaultProps}>
            {({ displayedFilters, showFilter }) => (
              <>
                ...
                />
                {Object.keys(displayedFilters).map((displayedFilter, index) => (
  ...
  describe('pagination', () => {
    it('should compute hasNextPage and hasPreviousPage based on total', async () => {
      const getList = jest.fn().mockImplementation(() => Promise.resolve({ data: [], total: 25 }));
      const dataProvider = testDataProvider({ getList });
      const children = jest.fn().mockReturnValue(<span>children</span>);
      const props = {
        ...defaultProps,
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <InfiniteListController disableSyncWithLocation {...props} />
        </CoreAdminContext>,
      );
      await waitFor(() => {
        expect(children).toHaveBeenCalledWith(
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <InfiniteListController resource="posts">
          {({ total }) => <div aria-label="Total value">{String(total)}</div>}
        </InfiniteListController>
      </CoreAdminContext>,
    );
