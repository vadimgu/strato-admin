...
export default {
  title: '@strato-admin/ra-core/controller/list/useList',
};
  ...
  return (
    <div>
      {listContext.isPending ? (
        ...
        <div>
          <div>
            <p data-testid="selected_ids">Selected ids: {JSON.stringify(listContext.selectedIds)}</p>
          </div>
          <ul
            style={{
              listStyleType: 'none',
          ...
          >
            {listContext.data?.map((record) => (
              <li key={record.id}>
                <input
                  type="checkbox"
                  checked={listContext.selectedIds.includes(record.id)}
                  onChange={() => listContext.onToggleItem(record.id)}
                  style={{
                    cursor: 'pointer',
                    marginRight: '10px',
                  }}
...
const SortButton = () => {
  ...
  return (
    <div style={{ display: 'flex', gap: '10px', margin: '10px' }}>
      <button
        onClick={() =>
          listContext.sort.field === 'id' && listContext.sort.order === 'ASC'
            ? listContext.setSort({ field: 'id', order: 'DESC' })
            : listContext.setSort({ field: 'id', order: 'ASC' })
        }
      >
        Sort by id {listContext.sort.field === 'id' && listContext.sort.order === 'ASC' ? 'DESC' : 'ASC'}
      </button>
      <button
        onClick={() =>
          listContext.sort.field === 'title' && listContext.sort.order === 'ASC'
            ? listContext.setSort({ field: 'title', order: 'DESC' })
            : listContext.setSort({ field: 'title', order: 'ASC' })
        }
      >
        Sort by title {listContext.sort.field === 'title' && listContext.sort.order === 'ASC' ? 'DESC' : 'ASC'}
      </button>
    </div>
  );
};
