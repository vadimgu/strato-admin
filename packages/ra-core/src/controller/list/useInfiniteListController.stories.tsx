...
import { InfiniteListControllerProps, useInfiniteListController } from './useInfiniteListController';
  ...
  <div style={styles.mainContainer}>
    {params.isPending ? (
      ...
      <div>
        ...
        >
          <button onClick={() => params.onSelectAll()} disabled={params.total === params.selectedIds.length}>
            Select All
          </button>
          <button onClick={() => params.onSelect([1])} disabled={params.selectedIds.includes(1)}>
            Select item 1
          </button>
          <button onClick={() => params.onSelectAll({ limit: 3 })} disabled={params.selectedIds.length >= 3}>
            Limited Select All
          </button>
          <button onClick={params.onUnselectItems} disabled={params.selectedIds.length === 0}>
            Unselect All
          </button>
          <p data-testid="selected_ids">Selected ids: {JSON.stringify(params.selectedIds)}</p>
        </div>
        <ul
          style={{
            listStyleType: 'none',
...
export const DisableAuthentication = ({
  ...
  return (
    <TestMemoryRouter initialEntries={['/posts']}>
      <CoreAdminContext dataProvider={dataProvider} authProvider={authProvider} dashboard={Dashboard}>
          ...
          <input
            ...
            onChange={() => {
              setAuthorizedResources((state) => ({
                ...state,
                'posts.list': !authorizedResources['posts.list'],
              }));
