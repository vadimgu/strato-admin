...
export const useRecordSelection = <RecordType extends RaRecord = any>(
  ...
  const [localSelectionStore, setLocalSelectionStore] = useState<RecordType['id'][]>(defaultIds);
  ...
  const [selectionStore, setSelectionStore] = useStore<RecordType['id'][]>(finalStoreKey, defaultIds);
  const [storeKeys, setStoreKeys] = useStore<string[]>(`${resource}.selectedIds.storeKeys`, defaultStoreKeys);
  ...
  const selectionModifiers = useMemo(
    () => ({
      ...
      unselect(idsToRemove: RecordType['id'][], fromAllStoreKeys?: boolean) {
      ...
      toggle: (id: RecordType['id']) => {
        ...
        setStore((ids) => {
          ...
          return hasId ? [...ids.slice(0, index), ...ids.slice(index + 1)] : [...ids, id];
        });
      },
        ...
        if (!disableSyncWithStore && fromAllStoreKeys) {
          storeKeys
            ...
            .forEach((storeKey) => {
              const ids = getItem<RecordType['id'][]>(storeKey);
              if (ids) {
                setItem<RecordType['id'][]>(storeKey, defaultIds);
              }
            });
        }
      },
    }),
    [disableSyncWithStore, finalStoreKey, getItem, setItem, setStore, storeKeys],
  );
