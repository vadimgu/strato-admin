...
export const WithListContext = <RecordType extends RaRecord>({
  ...
  if (isPaused && (isPending || isPlaceholderData) && offline !== false && offline !== undefined) {
    return offline;
  }
  ...
  if ((data == null || data.length === 0 || total === 0) && empty !== false && empty !== undefined) {
    return empty;
  }
...
export interface WithListContextProps<RecordType extends RaRecord> extends React.PropsWithChildren<
  Partial<Pick<ListControllerResult<RecordType>, 'data' | 'total' | 'isPending'>>
> {
