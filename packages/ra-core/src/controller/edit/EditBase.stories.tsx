...
export const WithAuthProviderNoAccessControl = ({
  ...
  <CoreAdminContext authProvider={authProvider} dataProvider={dataProvider}>
    <EditBase {...defaultProps} {...EditProps} authLoading={<div>Authentication loading...</div>}>
      <Child />
    </EditBase>
  </CoreAdminContext>
);
...
export const AccessControl = ({
  ...
  <CoreAdminContext authProvider={authProvider} dataProvider={dataProvider}>
    <EditBase {...defaultProps} authLoading={<div>Authentication loading...</div>}>
      <Child />
    </EditBase>
  </CoreAdminContext>
);
  ...
  const dataProvider = {
    ...
    getOne: (resource, params) => {
      return new Promise<GetOneResult>((resolve) => {
        resolveGetOne = () => resolve(defaultDataProvider.getOne(resource, params));
      });
    },
  };
  ...
  return (
    <CoreAdminContext dataProvider={dataProvider}>
      <EditBase
        ...
        offline={<p style={{ color: 'orange' }}>You are offline, cannot load data</p>}
      >
        <OfflineChild />
      </EditBase>
    </CoreAdminContext>
...
const Child = ({ callTimeOptions }: { callTimeOptions?: SaveHandlerCallbacks }) => {
...
const OfflineChild = ({ callTimeOptions }: { callTimeOptions?: SaveHandlerCallbacks }) => {
  const saveContext = useSaveContext();
...
const MutationsState = () => {
  ...
  return (
    <IsOffline>
      {pendingMutations.length > 0 ? <p>You have pending mutations</p> : <p>No pending mutations</p>}
    </IsOffline>
  );
};
