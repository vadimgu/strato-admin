...
describe('EditBase', () => {
  ...
  it('should give access to the save function', async () => {
    const dataProvider = testDataProvider({
      ...
      update: jest.fn((_, { id, data, previousData }) => Promise.resolve({ data: { id, ...previousData, ...data } })),
    });

    render(<NoAuthProvider dataProvider={dataProvider} mutationMode="pessimistic" />);
  ...
  it('should allow to override the onSuccess function', async () => {
    ...
    render(<NoAuthProvider dataProvider={dataProvider} mutationMode="pessimistic" mutationOptions={{ onSuccess }} />);
  ...
  it('should allow to override the onError function', async () => {
    ...
    render(<NoAuthProvider dataProvider={dataProvider} mutationMode="pessimistic" mutationOptions={{ onError }} />);
  ...
  it('should allow to override the transform function', async () => {
    ...
    render(<NoAuthProvider dataProvider={dataProvider} mutationMode="pessimistic" transform={transform} />);

    await screen.findByText('previous');
    fireEvent.click(screen.getByText('save'));

    await waitFor(() => {
      expect(transform).toHaveBeenCalledWith({ test: 'test' }, { previousData: { id: 12, test: 'previous' } });
    });
    await waitFor(() => {
      expect(dataProvider.update).toHaveBeenCalledWith('posts', {
        id: 12,
  ...
  it('should allow to override the transform function at call time', async () => {
    ...
    await waitFor(() => {
      expect(transformOverride).toHaveBeenCalledWith({ test: 'test' }, { previousData: { id: 12, test: 'previous' } });
    });
    await waitFor(() => {
      expect(dataProvider.update).toHaveBeenCalledWith('posts', {
        id: 12,
  ...
  it('should load data immediately if authProvider is not provided', async () => {
    const dataProvider = testDataProvider({
      // @ts-ignore
      getOne: jest.fn(() => Promise.resolve({ data: { id: 12, test: 'Hello' } })),
    });
  ...
  it('should wait for the authentication resolution before loading data', async () => {
    ...
    render(<WithAuthProviderNoAccessControl authProvider={authProvider} dataProvider={dataProvider} />);
  ...
  it('should wait for both the authentication and authorization resolution before loading data', async () => {
    ...
    render(<AccessControl authProvider={authProvider} dataProvider={dataProvider} />);
    expect(dataProvider.getOne).not.toHaveBeenCalled();
    await screen.findByText('Authentication loading...');
    resolveAuth!();
    expect(dataProvider.getOne).not.toHaveBeenCalled();
  ...
  it('should allow renderProp', async () => {
    ...
    render(<WithRenderProps dataProvider={dataProvider} mutationMode="pessimistic" />);
