...
import { SaveContextValue, SaveHandlerCallbacks, useMutationMiddlewares } from '../saveContext';
 ...
 * }
 */
export const useEditController = <RecordType extends RaRecord = any, ErrorType = Error>(
  ...
  const { meta: mutationMeta, onSuccess, onError, ...otherMutationOptions } = mutationOptions;
  ...
  const { registerMutationMiddleware, getMutateWithMiddlewares, unregisterMutationMiddleware } =
            ...
            _:
              typeof error === 'string'
                ...
                : error instanceof Error ||
                    (typeof error === 'object' && error !== null && error.hasOwnProperty('message'))
                  ? // @ts-ignore
                    error.message
                  : undefined,
          },
  ...
  const save = useCallback(
    ...
    [id, mutationMeta, record, resource, transform, update, recordCached.previousData],
  );
...
export interface EditControllerLoadingResult<
...
export interface EditControllerSuccessResult<
  RecordType extends RaRecord = any,
> extends EditControllerBaseResult<RecordType> {
  record: RecordType;
  error: null;
...
export type EditControllerResult<RecordType extends RaRecord = any, ErrorType = Error> =
