...
import { EditContextProvider, SaveContextProvider, useEditController } from '..';
...
import { Authenticated, CanAccess, DisableAuthentication } from './useEditController.security.stories';
import { EncodedId, WarningLogWithDifferentMeta } from './useEditController.stories';
...
describe('useEditController', () => {
  ...
  it('should call the dataProvider.getOne() function on mount', async () => {
    const getOne = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: { id: 12, title: 'hello' } }));
    const dataProvider = { getOne } as unknown as DataProvider;
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <EditController {...defaultProps}>{({ record }) => <div>{record && record.title}</div>}</EditController>
      </CoreAdminContext>,
    );
    await waitFor(() => {
  ...
  it.each([
    ...
    const getOne = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: { id, title: 'hello' } }));
    const dataProvider = { getOne } as unknown as DataProvider;
  ...
  it('should use the id provided through props if any', async () => {
    const getOne = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: { id: 0, title: 'hello' } }));
    const dataProvider = { getOne } as unknown as DataProvider;
  ...
  it('should return the `redirect` provided through props or the default', async () => {
    const getOne = jest.fn().mockImplementationOnce(() => Promise.resolve({ data: { id: 12, title: 'hello' } }));
    const dataProvider = { getOne } as unknown as DataProvider;
    const Component = ({ redirect = undefined }: { redirect?: RedirectionSideEffect }) => (
      <CoreAdminContext dataProvider={dataProvider}>
  ...
  describe('queryOptions', () => {
    it('should accept custom client query options', async () => {
      ...
      const getOne = jest.fn().mockImplementationOnce(() => Promise.reject(new Error()));
      const onError = jest.fn();
      const dataProvider = { getOne } as unknown as DataProvider;

      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <EditController {...defaultProps} resource="posts" queryOptions={{ onError }}>
            {() => <div />}
          </EditController>
        </CoreAdminContext>,
      );
    ...
    it('should accept a meta in query options', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <EditController {...defaultProps} resource="posts" queryOptions={{ meta: { foo: 'bar' } }}>
            {() => <div />}
          </EditController>
        </CoreAdminContext>,
      );
  ...
  it('should call the dataProvider.update() function on save', async () => {
    ...
    const dataProvider = {
      getOne: () => Promise.resolve({ data: { id: 12, test: 'previous' } }),
      update,
        ...
        <EditController {...defaultProps} mutationMode="pessimistic">
          {({ record, save }) => {
            return (
              <>
                <p>{record?.test}</p>
                <button aria-label="save" onClick={() => save!({ test: 'updated' })} />
              </>
            );
          }}
        </EditController>
  ...
  it('should return an undoable save callback by default', async () => {
    ...
    const update = jest.fn().mockImplementationOnce((_, { data, previousData }) => {
  ...
  it('should execute success side effects on success in pessimistic mode', async () => {
    ...
    const dataProvider = {
      ...
      update: (_, { id, data, previousData }) => Promise.resolve({ data: { id, ...previousData, ...data } }),
    } as unknown as DataProvider;
  ...
  describe('mutationOptions', () => {
    it('should allow mutationOptions to override the default success side effects in pessimistic mode', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Notification />
          <EditController {...defaultProps} mutationMode="pessimistic" mutationOptions={{ onSuccess }}>
            {({ record, save }) => {
              saveCallback = save;
    ...
    it('should allow mutationOptions to override the default success side effects in optimistic mode', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Notification />
          <EditController {...defaultProps} mutationMode="optimistic" mutationOptions={{ onSuccess }}>
            {({ record, save }) => {
              saveCallback = save;
              return <div>{record?.id}</div>;
            }}
    ...
    it('should allow mutationOptions to override the default success side effects in undoable mode', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Notification />
          <EditController {...defaultProps} mutationOptions={{ onSuccess }}>
            {({ record, save }) => {
              saveCallback = save;
              return <div>{record?.id}</div>;
            }}
    ...
    it('should allow mutationOptions to override the default failure side effects in pessimistic mode', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Notification />
          <EditController {...defaultProps} mutationMode="pessimistic" mutationOptions={{ onError }}>
            {({ save }) => {
              saveCallback = save;
              return <div />;
            }}
    ...
    it('should allow mutationOptions to override the default failure side effects in optimistic mode', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <Notification />
          <EditController {...defaultProps} mutationMode="optimistic" mutationOptions={{ onError }}>
            {({ save }) => {
              saveCallback = save;
              return <div />;
            }}
    ...
    it('should accept meta in mutationOptions', async () => {
      ...
      render(
        <CoreAdminContext dataProvider={dataProvider}>
          <EditController {...defaultProps} mutationMode="pessimistic" mutationOptions={{ meta: { lorem: 'ipsum' } }}>
            {({ save }) => {
              saveCallback = save;
              return <div />;
            }}
  ...
  it('should accept meta as a save option', async () => {
    ...
    await act(async () => saveCallback({ foo: 'bar' }, { meta: { lorem: 'ipsum' } }));
    await waitFor(() => {
      expect(update).toHaveBeenCalledWith('posts', {
        id: 12,
        data: { foo: 'bar' },
  ...
  it('should allow transform to transform the data before save', async () => {
    let saveCallback;
    const update = jest.fn().mockImplementationOnce((_, { id, data }) => Promise.resolve({ data: { id, ...data } }));
    const dataProvider = {
      getOne: () => Promise.resolve({ data: { id: 12 } }),
      update,
    } as unknown as DataProvider;
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <EditController {...defaultProps} mutationMode="pessimistic" transform={transform}>
          {({ save }) => {
            saveCallback = save;
            return <div />;
          }}
    ...
    await act(async () => saveCallback({ foo: 'bar' }));
    expect(transform).toHaveBeenCalledWith({ foo: 'bar' }, { previousData: undefined });
  ...
  it('should allow the save transform option to override the transform side effect', async () => {
    ...
    expect(transformSave).toHaveBeenCalledWith({ foo: 'bar' }, { previousData: undefined });
    expect(update).toHaveBeenCalledWith('posts', {
      id: 12,
      data: { foo: 'bar', transformed: true },
      previousData: undefined,
  ...
  it('should allow to register middlewares', async () => {
    ...
    const middleware: Middleware<DataProvider['update']> = jest.fn((resource, params, next) => {
    ...
    render(
      <CoreAdminContext dataProvider={dataProvider}>
        <EditController {...defaultProps} mutationMode="pessimistic">
          {({ save, saving, registerMutationMiddleware, unregisterMutationMiddleware }) => {
            saveCallback = save;
            return (
              <SaveContextProvider
                value={{
  ...
  it('should allow custom redirect with warnWhenUnsavedChanges in pessimistic mode', async () => {
    const dataProvider = testDataProvider({
      ...
      update: (_, { id, data }) =>
        new Promise((resolve) => setTimeout(() => resolve({ data: { id, ...data } } as any), 300)),
    });
  ...
  describe('security', () => {
    it('should not call the dataProvider until the authentication check passes', async () => {
      ...
      render(<Authenticated authProvider={authProvider} dataProvider={dataProvider} />);
      await waitFor(() => {
    ...
    it('should call the dataProvider if disableAuthentication is true', async () => {
      ...
      render(<DisableAuthentication authProvider={authProvider} dataProvider={dataProvider} />);
      await screen.findByText('A post - 0 votes');
