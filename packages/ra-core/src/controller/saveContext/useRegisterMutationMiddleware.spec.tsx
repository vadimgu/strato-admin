...
import { SaveContextProvider, useMutationMiddlewares, useRegisterMutationMiddleware } from '../..';
    ...
    const Parent = () => {
      ...
      return (
        <SaveContextProvider value={middlewaresFunctions}>
          {mount && <Middleware />}
          <button onClick={() => setMount(!mount)}>Toggle middleware</button>
          <button
            onClick={() => {
              const saveWithMiddlewares = middlewaresFunctions.getMutateWithMiddlewares(save);
              saveWithMiddlewares();
            }}
          >
            Save
