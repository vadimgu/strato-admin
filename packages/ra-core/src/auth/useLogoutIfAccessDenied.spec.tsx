...
const authProvider: AuthProvider = {
  ...
  checkAuth: () => (loggedIn ? Promise.resolve() : Promise.reject('bad method')),
  checkError: (params) => {
    if (params instanceof Error) {
      return Promise.reject(new Error(params.message === 'denied' ? 'logout' : params.message));
    }
    return Promise.resolve();
  },
...
describe('useLogoutIfAccessDenied', () => {
  ...
  it('should not log out if passed an error that does not make the authProvider throw', async () => {
    render(<Route path="/" element={<TestComponent error={new Error()} />} />, {
      wrapper: TestWrapper,
    });
    await waitFor(() => {
  ...
  it('should logout if passed an error that makes the authProvider throw', async () => {
    render(<Route path="/" element={<TestComponent error={new Error('denied')} />} />, {
      wrapper: TestWrapper,
    });
    await waitFor(() => {
  ...
  it('should logout without showing a notification if it has been called with error param', async () => {
    render(<Route path="/" element={<TestComponent error={new Error('Error')} />} />, {
      wrapper: TestWrapper,
    });
    await waitFor(() => {
      expect(authProvider.logout).toHaveBeenCalledTimes(0);
  ...
  it('should notify if passed an error with a message that makes the authProvider throw', async () => {
    render(<Route path="/" element={<TestComponent error={new Error('Test message')} />} />, {
      wrapper: TestWrapper,
    });
    await waitFor(() => {
      expect(authProvider.logout).toHaveBeenCalledTimes(1);
