...
export const WithAuthProviderAndGetPermissions = ({
  authProvider = {
    ...
    checkAuth: () => Promise.reject('bad method'),
    checkError: () => Promise.reject('bad method'),
    getPermissions: () => new Promise((resolve) => setTimeout(resolve, 300, 'admin')),
  },
  queryClient,
}: {
...
const UsePermissions = ({ children }: { children: (state: UsePermissionsResult<any, Error>) => React.ReactNode }) => {
...
const inspectState = (state: UsePermissionsResult<any, Error>) => (
  <div>
    ...
    {state.permissions ? <span>PERMISSIONS: {state.permissions}</span> : null}
