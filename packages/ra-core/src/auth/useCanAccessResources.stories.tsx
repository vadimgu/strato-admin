...
import { useCanAccessResources, UseCanAccessResourcesResult } from './useCanAccessResources';
...
const StateInspector = ({ state }: { state: UseCanAccessResourcesResult }) => {
  return (
    <div>
      ...
      {state.canAccess !== undefined && <span>{JSON.stringify(state.canAccess)}</span>}
...
export const Basic = ({
  ...
  <CoreAdminContext authProvider={authProvider != null ? authProvider : undefined} queryClient={queryClient}>
    <UseCanAccessResources action="read" resources={['posts.id', 'posts.title', 'posts.author']}>
