...
>(
  ...
  const queryResult = useQuery({
    queryKey: ['auth', 'canAccess', { ...restParams, recordId: record?.id, resource }],
  ...
  return authProviderHasCanAccess ? result : (emptyQueryObserverResult as unknown as UseCanAccessResult<ErrorType>);
};
...
export interface UseCanAccessLoadingErrorResult<ErrorType = Error> extends QueryObserverLoadingErrorResult<
  boolean,
  ErrorType
> {
  canAccess: undefined;
}
export interface UseCanAccessRefetchErrorResult<ErrorType = Error> extends QueryObserverRefetchErrorResult<
  boolean,
  ErrorType
> {
  canAccess: boolean;
