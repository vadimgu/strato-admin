...
import { Basic, CustomLoading, CustomAccessDenied, NoAuthProvider, AccessDenied } from './CanAccess.stories';
...
describe('<CanAccess>', () => {
  ...
  it('shows nothing by default when users are denied access', async () => {
    ...
    const { container } = render(<AccessDenied authProvider={authProvider} />);
    expect(container.textContent).toEqual('');
    expect(screen.queryByText('protected content')).toBeNull();
  ...
  it('shows the custom accessDenied element when users are denied access', async () => {
    ...
    const { container } = render(<CustomAccessDenied authProvider={authProvider} />);
    expect(container.textContent).toEqual('');
    resolveCheckAuth(false);
  ...
  it('redirects to the /authentication-error route by default in case of error', async () => {
    ...
    await waitFor(() => expect(location.pathname).toEqual('/authentication-error'));
  });
  it('redirects to the /authentication-error route by default in case of error in an Admin with a basename', async () => {
    ...
    expect(container.textContent).toEqual('');
    rejectCheckAuth(new Error('failed'));
    await waitFor(() => expect(location.pathname).toEqual('/admin/authentication-error'));
  });
