...
const DefaultRemoveItemButton = (
  props: Omit<React.DetailedHTMLProps<React.ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>, 'onClick'>,
) => {
  ...
  return (
    <button
      type="button"
      onClick={() => remove()}
      className={[`button-remove button-remove-${source}-${index}`, className].join(' ')}
      {...rest}
    >
...
const DefaultReOrderButtons = ({ className }: { className?: string }) => {
  ...
  return (
    <span className={[`button-reorder button-reorder-${source}-${index}`, className].join(' ')}>
      <button type="button" onClick={() => reOrder(index - 1)} disabled={index <= 0}>
        <Translate i18nKey="ra.action.move_up">Move Up</Translate>
      </button>
      <button type="button" onClick={() => reOrder(index + 1)} disabled={total == null || index >= total - 1}>
...
export const SimpleFormIterator = (props: SimpleFormIteratorProps) => {
  ...
  return fields ? (
    <SimpleFormIteratorBase getItemDefaults={getItemDefaults} {...props}>
      <div className={[className, fullWidth ? 'fullwidth' : '', disabled ? 'disabled' : ''].join(' ')}>
        <ul
          style={{
            listStyle: 'none',
            margin: 0,
          ...
          >
            ...
            {fields.length > 0 && !disableClear && !disableRemove && (
              <div>
                ...
                />
                <button type="button" onClick={() => setConfirmIsOpen(true)}>
                  <Translate i18nKey="ra.action.clear_array_input">Clear</Translate>
                </button>
              </div>
            )}
          </div>
