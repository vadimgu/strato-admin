...
export const AutocompleteInput = (props: Partial<InputProps> & Partial<ChoicesProps> & { multiple?: boolean }) => {
  const { allChoices, source, setFilters, filterValues } = useChoicesContext(props);
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => setFilters({ ...filterValues, q: e.target.value })}
      />
      <button
        type="button"
        onClick={(event) => {
          ...
          <li key={choice.id}>
            <label>
              <input
                ...
                onChange={(event) => {
                  const newValue = event.target.checked
                    ? props.multiple
                      ...
                      : choice.id
                    : props.multiple
                      ? field.value.filter((v: any) => v !== choice.id)
                      : null;
                ...
                aria-describedby={fieldState.error ? `error-${props.source}` : undefined}
              />
