...
describe('getFieldLabelTranslationArgs', () => {
  it('should return empty span by default', () => expect(getFieldLabelTranslationArgs()).toEqual(['']));
  ...
  it('should return the humanized source when given', () => {
    ...
    ).toEqual(['resources.posts.fields.title_with_underscore', { _: 'Title with underscore' }]);
    ...
    ).toEqual(['resources.posts.fields.title.with.dots', { _: 'Title with dots' }]);
    ...
    ).toEqual(['resources.posts.fields.titleWithCamelCase', { _: 'Title with camel case' }]);
  });
  ...
  it('should ignore the source part corresponding to the parent in an iterator', () => {
    ...
    ).toEqual(['resources.posts.fields.book.authors.categories.identifier.name', { _: 'Identifier name' }]);
  });
