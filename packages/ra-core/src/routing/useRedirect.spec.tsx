...
describe('useRedirect', () => {
    ...
    render(
      <TestMemoryRouter>
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Routes>
            <Route path="/" element={<Redirect redirectTo="/foo?bar=baz" />} />
  ...
  it('should redirect to the path with state', async () => {
    render(
      <TestMemoryRouter>
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Routes>
            <Route path="/" element={<Redirect redirectTo="/foo" state={{ bar: 'baz' }} />} />
            <Route path="/foo" element={<Component />} />
          </Routes>
        </CoreAdminContext>
      </TestMemoryRouter>,
    ...
    await waitFor(() => {
      expect(screen.queryByDisplayValue(JSON.stringify({ _scrollToTop: true, bar: 'baz' }))).not.toBeNull();
    });
  });
  ...
  it('should support functions that returns local absolute URLs with a leading /', async () => {
    ...
    fireEvent.click(await screen.findByText('Relative url from a function'));
    await screen.findByText('Admin dashboard');
  });
});
