...
export default {
  title: '@strato-admin/ra-core/routing/useGetPathForRecord',
};

const EditLink = () => {
  const path = useGetPathForRecord({ link: 'edit' });
  return path ? <LinkBase to={path}>Edit</LinkBase> : <span>Edit no link</span>;
};

const ShowLink = () => {
  const path = useGetPathForRecord({ link: 'show' });
  return path ? <LinkBase to={path}>Show</LinkBase> : <span>Show no link</span>;
};
...
export const AccessControlWithLinkTypeProvided = ({
  authProvider = {
    ...
    canAccess: ({ action }) => new Promise((resolve) => setTimeout(resolve, 300, action === 'edit')),
  },
}: {
  authProvider?: AuthProvider;
}) => (
  <TestMemoryRouter>
    <CoreAdminContext queryClient={new QueryClient()} authProvider={authProvider}>
      <ResourceContextProvider value="posts">
        <RecordContextProvider value={{ id: 123 }}>
          <div style={{ display: 'flex', gap: 2 }}>
            <EditLink />
  ...
  <TestMemoryRouter>
    <CoreAdminContext
      ...
      authProvider={{
        ...
        checkError: () => Promise.resolve(),
        getPermissions: () => Promise.resolve(),
        canAccess: ({ action }) => new Promise((resolve) => setTimeout(resolve, 300, action === 'show')),
      }}
    >
      <ResourceContextProvider value="posts">
        <ResourceDefinitionContextProvider
