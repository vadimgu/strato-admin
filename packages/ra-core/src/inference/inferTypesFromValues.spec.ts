...
describe('inferTypeFromValues', () => {
  ...
  it('should return a date field for date values', () => {
    expect(inferTypeFromValues('foo', [new Date('2018-10-01'), new Date('2018-12-03')])).toEqual({
      type: 'date',
      props: { source: 'foo' },
    });
  });
  ...
  it('should return an email field for email string values', () => {
    expect(inferTypeFromValues('foo', ['me@example.com', 'you@foo.co.uk'])).toEqual({
      type: 'email',
      props: { source: 'foo' },
    });
  });
  ...
  it('should return a url field for url string values', () => {
    expect(inferTypeFromValues('foo', ['http://foo.com/bar', 'https://www.foo.com/index.html#foo'])).toEqual({
      type: 'url',
      props: { source: 'foo' },
    });
  });
  it('should return a date field for date string values', () => {
    expect(inferTypeFromValues('foo', ['2018-10-01', '2018-12-03'])).toEqual({
      type: 'date',
      props: { source: 'foo' },
    });
  });
  it('should return a rich text field for HTML values', () => {
    expect(inferTypeFromValues('foo', ['This is <h1>Good</h1>', '<body><h1>hello</h1>World</body>'])).toEqual({
      type: 'richText',
      props: { source: 'foo' },
    });
  });
  it('should return a string field for string values', () => {
    expect(inferTypeFromValues('foo', ['This is Good', 'hello, World!'])).toEqual({
      type: 'string',
      props: { source: 'foo' },
    });
  });
