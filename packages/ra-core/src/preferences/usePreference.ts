...
function usePreference(): [unknown, (value: unknown | ((value: unknown) => void), defaultValue?: unknown) => void];
function usePreference<T>(key = '', defaultValue = undefined) {
  ...
  return useStore<T>(preferenceKey && key ? `${preferenceKey}.${key}` : (preferenceKey ?? key), defaultValue);
}
