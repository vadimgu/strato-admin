...
import { getRecordFromLocation, useRecordFromLocation } from './useRecordFromLocation';
...
describe('useRecordFromLocation', () => {
  ...
  it('return the record from the location search', async () => {
    ...
    render(
      <TestMemoryRouter initialEntries={[`/posts/create?source=${JSON.stringify(record)}`]}>
        <UseGetRecordFromLocation />
      </TestMemoryRouter>,
    );
  ...
  it('return the record from the location state', async () => {
    const record = { test: 'value' };
    render(
      <TestMemoryRouter initialEntries={[{ pathname: `/posts/create`, state: { record } }]}>
        <UseGetRecordFromLocation />
      </TestMemoryRouter>,
    );
