...
export default {
  title: '@strato-admin/ra-core/form/validation/useUnique',
};
...
const OrgSelect = (props) => {
  ...
  return (
    <>
      <select aria-label="organization" aria-invalid={fieldState.invalid} {...field}>
...
const Wrapper = ({ children, dataProvider = defaultDataProvider }) => {
  return (
    <TestMemoryRouter>
      <CoreAdminContext dataProvider={dataProvider} i18nProvider={i18nProvider} queryClient={new QueryClient()}>
        {children}
      </CoreAdminContext>
    </TestMemoryRouter>
  );
...
const BasicForm = () => {
  ...
  return (
    <Form defaultValues={{ name: 'John Doe' }}>
      <p>The name field should be unique. Try to enter "John Doe" or "Jane Doe".</p>
...
const EditForm = () => {
  ...
  return (
    <Form defaultValues={{ name: 'John Doe' }}>
      <p>
        The name field should be unique. Try to enter "John Doe". Jane Doe should work as this is the current record
        value
      </p>
      <Input source="name" defaultValue="" validate={unique()} />
      <button type="submit">Submit</button>
...
export const Edit = ({ dataProvider, id = 2 }: { dataProvider?: DataProvider; id?: number }) => {
  return (
    <Wrapper dataProvider={dataProvider}>
...
export const WithMessage = ({ dataProvider }: { dataProvider?: DataProvider }) => {
  return (
    <Wrapper dataProvider={dataProvider}>
      <CreateBase resource="users">
        <WithMessageForm />
...
const WithAdditionalFiltersForm = () => {
  ...
  return (
    <Form defaultValues={{ name: 'John Doe', organization_id: 1 }}>
      <p>
        The name field should be unique. "John Doe" is already registered for BigCorp and "Jane Doe" is already
        registered for EvilCorp.
      </p>
...
export const WithAdditionalFilters = ({ dataProvider }: { dataProvider?: DataProvider }) => {
  return (
    <Wrapper dataProvider={dataProvider}>
      <CreateBase resource="users">
