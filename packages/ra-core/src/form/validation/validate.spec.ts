...
describe('Validators', () => {
  const test = async (validator, inputs, message) => {
    const validationResults = await Promise.all<Error | undefined>(inputs.map((input) => validator(input, null))).then(
      (results) => results.map((error) => (error && error.message ? error.message : error)),
    );

    expect(validationResults).toEqual(Array(...Array(inputs.length)).map(() => message));
  };
  ...
  describe('combine2Validators', () => {
    it('should create a new validator that always return the result directly when both validator are synchronous', () => {
      const includesFoo = (value) => (value.match(/foo/) ? null : 'value must include foo');
      const includesBar = (value) => (value.match(/bar/) ? null : 'value must include bar');

      const combinedValidator = combine2Validators(includesFoo, includesBar);
      expect(combinedValidator('foobar', null, null)).toBe(null);
      expect(combinedValidator('bar', null, null)).toBe('value must include foo');
      expect(combinedValidator('foo', null, null)).toBe('value must include bar');
      expect(combinedValidator('', null, null)).toBe('value must include foo');
    });

    it('should create a new validator that always return a promise when both validator are asynchronous', async () => {
      const includesFoo = (value) => Promise.resolve(value.match(/foo/) ? null : 'value must include foo');
      const includesBar = (value) => Promise.resolve(value.match(/bar/) ? null : 'value must include bar');
  ...
  describe('composeValidators', () => {
    const asyncSuccessfullValidator = async () => new Promise((resolve) => resolve(undefined));
    const asyncFailedValidator = async () => new Promise((resolve) => resolve('async'));

    it('Correctly composes validators passed as an array', async () => {
      await test(
        composeValidators([required(), minLength(5), asyncSuccessfullValidator]),
        [''],
        'ra.validation.required',
      );
      await test(
        composeValidators([required(), asyncSuccessfullValidator, minLength(5)]),
        ['abcd'],
        'ra.validation.minLength',
      );
      await test(composeValidators([required(), asyncFailedValidator, minLength(5)]), ['abcde'], 'async');
      await test(composeValidators([required(), minLength(5), asyncSuccessfullValidator]), ['abcde'], undefined);
    });
