...
describe('useInput', () => {
    ...
    render(
      <CoreAdminContext dataProvider={testDataProvider()}>
        <Form onSubmit={jest.fn()}>
          <Input defaultValue="A title" source="title" resource="posts" validate={required()}>
            {(props) => {
              inputProps = props;
              return <div />;
            }}
  ...
  it('allows to extend the input event handlers', () => {
    ...
    render(
      <CoreAdminContext dataProvider={testDataProvider()}>
        <Form onSubmit={jest.fn()}>
          <Input source="title" resource="posts" onBlur={handleBlur} onChange={handleChange} defaultValue="">
            {({ id, field }) => {
              return <input type="text" id={id} aria-label="Title" {...field} />;
            }}
          </Input>
        </Form>
      </CoreAdminContext>,
      ...
      <CoreAdminContext dataProvider={testDataProvider()}>
        <Form onSubmit={jest.fn()}>
          <InputWithCustomOnChange
            ...
            defaultValue=""
          >
            {({ id, field }) => <input type="text" id={id} aria-label="Title" {...field} />}
          </InputWithCustomOnChange>
        </Form>
      </CoreAdminContext>,
    );
  ...
  describe('defaultValue', () => {
        ...
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Form onSubmit={jest.fn()} record={{ id: 1, views: 0 }}>
            <Input source="views" resource="posts" defaultValue={99}>
              {({ id, field }) => {
                return <input type="number" id={id} aria-label="Views" {...field} />;
              }}
            </Input>
          </Form>
        </CoreAdminContext>,
    ...
    const BooleanInput = ({ source, defaultValue }: { source: string; defaultValue?: boolean }) => (
    ...
    const BooleanInputValue = ({ source }) => {
      ...
      return <>{typeof values[source] === 'undefined' ? 'undefined' : values[source] ? 'true' : 'false'}</>;
    };
    ...
    it('does not apply the defaultValue true when the field is of type checkbox and has a value', () => {
      render(
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Form record={{ id: 1, is_published: false }} onSubmit={jest.fn()}>
            <BooleanInput source="is_published" defaultValue={true} />
          </Form>
        </CoreAdminContext>,
      );
    ...
    it('does not apply the defaultValue false when the field is of type checkbox and has a value', () => {
      render(
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Form record={{ id: 1, is_published: true }} onSubmit={jest.fn()}>
            <BooleanInput source="is_published" defaultValue={false} />
          </Form>
        </CoreAdminContext>,
      );
      ...
      render(
        <CoreAdminContext dataProvider={testDataProvider()}>
          <Form onSubmit={jest.fn()} mode="onChange">
            <Input defaultValue="A title" source="title" resource="posts" validate={validator}>
              {(props) => <input type="text" onChange={props.field.onChange} data-testid="title-input" />}
            </Input>
            <Input defaultValue="A description" source="description" resource="posts">
              {() => <div />}
            </Input>
          </Form>
        </CoreAdminContext>,
      ...
      const DisabledEnableInput = () => {
        ...
        return (
          <>
            ...
            <Input source="title" resource="posts" validate={required()} disabled={disabled}>
              {(props) => {
                inputProps = props; // Capture the latest props
                return <input type="text" id={props.id} aria-label="Title" {...props.field} />;
              }}
            </Input>
          </>
        );
