...
describe('<FilterLiveForm />', () => {
  ...
  it('should work when users type at the exact same pace than the debounce delay', async () => {
    render(<Basic ListBaseProps={{ debounce: 100 }} FilterLiveFormProps={{ debounce: 100 }} />);
    const input = await screen.findByLabelText('title');
  ...
  it('should allow to clear a filter value with parse/format', async () => {
    ...
    await screen.findByText('{"category":"deals","document":"123123123123"}');
    fireEvent.change(input, { target: { value: '' } });
    await screen.findByText('{"category":"deals"}');
  });
  ...
  it('should support having multiple inputs', async () => {
    ...
    await screen.findByText('{"category":"deals","title":"foo","author":"bar"}');
  ...
  it('should support per input validation', async () => {
    ...
    const titleInput = await screen.findByLabelText('title');
    fireEvent.change(titleInput, { target: { value: 'foo' } });
    await screen.findByText('{"category":"deals","author":"Leo Tolstoy","title":"foo"}');
    const authorInput = await screen.findByLabelText('author');
    ...
    await screen.findByText('{"category":"deals","author":"valid","title":"new changes"}');
    expect(screen.queryByText('@@react-admin@@"ra.validation.required"')).toBeNull();
  });
  ...
  it('should not reapply old externally applied filters after clear', async () => {
    ...
    await waitFor(() => {
      expect(JSON.parse(screen.queryByTestId('filter-values')?.textContent || '')).toEqual({
        body: 'foo',
      });
    });
    // Unmount
  ...
  describe('getFilterFormValues', () => {
    it('should correctly get the filter form values from the new filterValues', () => {
      ...
      expect(getFilterFormValues(currentFormValues, newFilterValues)).toEqual({
        classicToClear: '',
