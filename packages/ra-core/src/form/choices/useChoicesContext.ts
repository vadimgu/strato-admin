...
export const useChoicesContext = <ChoicesType extends RaRecord = RaRecord>(
  ...
  const context = useContext(ChoicesContext) as ChoicesContextValue<ChoicesType>;
  const choices =
    options.choices && isArrayOfStrings(options.choices) ? convertOptionsToChoices(options.choices) : options.choices;
