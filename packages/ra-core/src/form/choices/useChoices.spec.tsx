...
describe('useChoices hook', () => {
  ...
  const Component = ({ choice, optionText, optionValue, translateChoice }) => {
    ...
    return <div data-value={getChoiceValue(choice)}>{getChoiceText(choice)}</div>;
  };

  it('should use optionValue as value identifier', () => {
    render(<Component {...defaultProps} />);
    expect(screen.getByText('test').getAttribute('data-value')).toEqual('42');
  });
  ...
  it('should use optionText with a function value as text identifier', () => {
    render(<Component {...defaultProps} optionText={(choice) => choice.foobar} choice={{ id: 42, foobar: 'test' }} />);
    expect(screen.queryAllByText('test')).toHaveLength(1);
  });
  ...
  it('should use optionText with an element value as text identifier', () => {
    ...
    render(<Component {...defaultProps} optionText={<Foobar />} choice={{ id: 42, foobar: 'test' }} />);
    expect(screen.queryAllByText('test')).toHaveLength(1);
  });
