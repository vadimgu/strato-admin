...
import { AdminContext, BooleanInput, SimpleForm, TextInput, SimpleFormIterator, ArrayInput } from 'ra-ui-materialui';
...
describe('FormDataConsumerView', () => {
  ...
  it('should be reactive', async () => {
    render(
      <AdminContext dataProvider={testDataProvider()}>
        <ResourceContextProvider value="posts">
          <SimpleForm>
            <BooleanInput source="hi" defaultValue />
            <FormDataConsumer>{({ formData }) => (!formData.hi ? <TextInput source="bye" /> : null)}</FormDataConsumer>
          </SimpleForm>
        </ResourceContextProvider>
      </AdminContext>,
    );
    ...
    await waitFor(() => {
      expect(screen.queryByLabelText('resources.posts.fields.bye')).toBeNull();
    });

    fireEvent.click(screen.getByLabelText('resources.posts.fields.hi'));

    await waitFor(() => {
      expect(screen.getByLabelText('resources.posts.fields.bye')).not.toBeNull();
    });
  });
  ...
  it('calls its children with updated scopedFormData when inside an ArrayInput', async () => {
              ...
              <SimpleFormIterator>
                ...
                <FormDataConsumer>
                  {({ scopedFormData }) => {
                    ...
                    return scopedFormData && scopedFormData.name ? <TextInput source="role" /> : null;
                  }}
                </FormDataConsumer>
              </SimpleFormIterator>
            </ArrayInput>
    ...
    fireEvent.change(screen.getByLabelText('resources.posts.fields.authors.name'), {
      target: { value: 'a' },
    });
