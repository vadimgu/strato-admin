...
const Form = ({ onSubmit }) => {
  ...
  return (
    <form onSubmit={onSubmit}>
      ...
      <input {...form.register('firstName')} aria-labelledby="firstname-label" />
      <label id="author-label">Author</label>
      <input {...form.register('author.name')} aria-labelledby="author-label" />
...
describe('useWarnWhenUnsavedChanges', () => {
  ...
  test.each([
    ['simple', 'First Name'],
    ['nested', 'Author'],
  ])('should not warn when leaving form with submit button after updating %s field', async (_, field) => {
    window.confirm = jest.fn().mockReturnValue(true);
    render(<App />);
  ...
  test.each([
    ['simple', 'First Name'],
    ['nested', 'Author'],
  ])('should not warn when navigating from a sub page with submit button after updating %s field', async (_, field) => {
    window.confirm = jest.fn().mockReturnValue(true);
  ...
  test.each([
    ['simple', 'First Name'],
    ['nested', 'Author'],
  ])('should warn when leaving form with unsaved changes after updating %s field', (_, field) => {
    ...
    fireEvent.click(screen.getByText('Leave'));
    expect(window.confirm).toHaveBeenCalledWith('ra.message.unsaved_changes');
  ...
  test.each([
    ['simple', 'First Name'],
    ['nested', 'Author'],
  ])('should warn when leaving form with unsaved changes but accept override', async (_, field) => {
