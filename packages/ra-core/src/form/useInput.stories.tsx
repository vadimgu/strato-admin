...
export const DefaultValue = ({ initialValue }: { initialValue: string | null | undefined }) => {
  const [submittedData, setSubmittedData] = React.useState<any>();
  return (
    <CoreAdminContext>
      <Form record={{ field1: initialValue }} onSubmit={(data) => setSubmittedData(data)}>
        <div
          style={{
            display: 'flex',
            flexDirection: 'column',
...
export const Large = () => {
  ...
  const fields = Array.from({ length: 15 }).map((_, index) => (
    <Input key={index} source={`field${index + 1}`} label={`field${index + 1}`} />
  ));
  return (
    <CoreAdminContext>
      <Form
