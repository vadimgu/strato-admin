...
describe('Form', () => {
  const Input = (props) => {
    ...
    return (
      <>
        <input aria-label="name" type="text" aria-invalid={fieldState.invalid} {...field} />
  ...
  it('should keep dirty values when record changes and resetOptions.keepDirtyValues is true', async () => {
    const { rerender } = render(
      <CoreAdminContext>
        <Form onSubmit={jest.fn()} record={{ id: 1, name: 'Initial' }} resetOptions={{ keepDirtyValues: true }}>
          <Input source="name" />
        </Form>
      </CoreAdminContext>,
    );
  ...
  it('should NOT keep dirty values when switching to a different record even with resetOptions.keepDirtyValues', async () => {
    const { rerender } = render(
      <CoreAdminContext>
        <Form onSubmit={jest.fn()} record={{ id: 1, name: 'Record 1' }} resetOptions={{ keepDirtyValues: true }}>
          <Input source="name" />
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(screen.getByDisplayValue('User Modified Record 1')).not.toBeNull();
    });

    rerender(
      <CoreAdminContext>
        <Form onSubmit={jest.fn()} record={{ id: 2, name: 'Record 2' }} resetOptions={{ keepDirtyValues: true }}>
          <Input source="name" />
        </Form>
      </CoreAdminContext>,
    );
  ...
  it('Displays a notification on submit when invalid', async () => {
    const Notification = () => {
      ...
      return notifications.length > 0 ? <div>{notifications[0].message}</div> : null;
    };
  ...
  it('should set null or undefined values to null', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit}>
          <Input source="foo" parse={() => null} format={() => '23'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: null }, expect.anything());
    });
  });
  ...
  it('should set null or undefined deep values to null', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit}>
          <Input source="foo.bar" parse={() => null} format={() => '23'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: { bar: null } }, expect.anything());
    });
  });
  ...
  it('should accept string values', async () => {
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: str }, expect.anything());
    });
  });
  it('should accept date values', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit}>
          <Input source="foo" parse={() => date} format={() => 'date'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: date }, expect.anything());
    });
  });
  ...
  it('should accept array values', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit}>
          <Input source="foo" parse={() => arr} format={() => 'arr'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: arr }, expect.anything());
    });
  });
  ...
  it('should accept object values', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit}>
          <Input source="foo" parse={() => obj} format={() => 'obj'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({ foo: obj }, expect.anything());
    });
  });
  ...
  it('should accept adding objects in arrays', async () => {
    ...
    render(
      <CoreAdminContext>
        <Form defaultValues={{ foo: [{ foo: 1 }, { foo: 4 }] }} onSubmit={onSubmit}>
          <Input source="foo" parse={() => obj} format={() => 'obj'} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
  ...
  describe('defaultValues', () => {
    it('should use defaultValues when the record has no value for the field', () => {
      ...
      expect((screen.getByLabelText('name') as HTMLInputElement).value).toBe('bar');
    });
    it('should not use defaultValues when the record has a value for the field', () => {
      render(
        <CoreAdminContext>
          <Form defaultValues={{ foo: 'bar' }} record={{ id: 1, foo: 'hello' }}>
            <Input source="foo" />
          </Form>
        </CoreAdminContext>,
      );
      ...
      render(
        <CoreAdminContext>
          <Form defaultValues={defaultValues} onSubmit={onSubmit}>
            <Input source="foo" parse={() => values.foo} format={() => 'obj'} />
            <button type="submit">Submit</button>
          </Form>
        </CoreAdminContext>,
      );
    ...
    render(
      <CoreAdminContext>
        <Form onSubmit={onSubmit} resolver={yupResolver(schema)}>
          <Input source="title" />
          <Input source="number" aria-label="Number" defaultValue={-10} />
          <button type="submit">Submit</button>
        </Form>
      </CoreAdminContext>,
    );
  ...
  it('should support validation messages translations at the form level without warnings', async () => {
    ...
    expect(mock).not.toHaveBeenCalledWith(expect.stringContaining('Missing translation for key:'));
  ...
  it('should support using a custom global message with server side validation', async () => {
    ...
    fireEvent.click(screen.getByText('Submit'));
    await screen.findByText('Required');
    await screen.findByText('There are validation errors. Please fix them.');
  });
  ...
  ])('should support prefilling the from values from the location $from', async ({ url, expectedValue }) => {
    render(<MultiRoutesForm url={url} />);
    expect((await screen.findByLabelText<HTMLInputElement>('title')).value).toEqual('');
    expect((screen.getByText('Submit') as HTMLInputElement).disabled).toEqual(false);
    fireEvent.click(screen.getByText('Settings'));
    await screen.findByDisplayValue(expectedValue);
    expect(screen.getByText<HTMLInputElement>('Submit').disabled).toEqual(false);
  });
  it.each([
    {
      from: 'state',
    ...
    async ({ url, defaultValues, expectedValue, expectedDefaultValue }) => {
      ...
      expect((await screen.findByLabelText<HTMLInputElement>('category')).value).toEqual(expectedDefaultValue);
      expect((screen.getByText('Submit') as HTMLInputElement).disabled).toEqual(false);
      fireEvent.click(screen.getByText('Settings'));
      await screen.findByDisplayValue(expectedValue);
      expect(screen.getByText<HTMLInputElement>('Submit').disabled).toEqual(false);
