...
import { AdminContext, ArrayInput, SimpleForm, SimpleFormIterator, TextInput } from 'ra-ui-materialui';
...
describe('useFormGroup', () => {
  ...
  ])('should return a correct form group state when %s', (_, fieldStates, expectedGroupState) => {
  ...
  it('should return the correct group state when the group changes', async () => {
    ...
    const IsDirty = () => {
      ...
      return <button onClick={() => setGroup('simplegroup2')}>Change group</button>;
    };
  ...
  it('should return correct group state when an ArrayInput is in the group', async () => {
        ...
        <ResourceContextProvider value="posts">
          <SimpleForm>
            <FormGroupContextProvider name="backlinks">
              <IsDirty />
              <ArrayInput defaultValue={backlinksDefaultValue} source="backlinks">
                <SimpleFormIterator>
                  <TextInput source="url" />
    ...
    const addItemElement = screen.getByLabelText('ra.action.add').closest('button') as HTMLButtonElement;
