...
import { FormGroupsContext, FormGroupsContextValue, FormGroupSubscriber } from './FormGroupsContext';
...
export const FormGroupsProvider = ({ children }: { children: ReactNode }) => {
      ...
      registerField: (source, group) => {
        if (group != null) {
          if (!(formGroups.current[group] || []).includes(source)) {
            formGroups.current[group] = [...(formGroups.current[group] || []), source];
  ...
  return <FormGroupsContext.Provider value={formContextValue}>{children}</FormGroupsContext.Provider>;
};
