...
import { Route, Routes, useNavigate, Link, HashRouter, useLocation } from 'react-router-dom';
...
const Input = (props) => {
  ...
  return (
    ...
    >
      ...
      <input aria-label={field.name} id={field.name} type="text" aria-invalid={fieldState.invalid} {...field} />
      {fieldState.error && fieldState.error.message ? <ValidationError error={fieldState.error.message} /> : null}
    </div>
  );
};
...
export const Basic = () => {
  ...
  return (
    <CoreAdminContext>
      <Form onSubmit={(data) => setSubmittedData(data)} record={{ id: 1, field1: 'bar', field6: null }}>
        <Input source="field1" />
...
export const FormLevelValidation = ({ i18nProvider = defaultI18nProvider }: { i18nProvider?: I18nProvider }) => {
  const [submittedData, setSubmittedData] = React.useState<any>();
  return (
...
export const InputLevelValidation = ({ i18nProvider = defaultI18nProvider }: { i18nProvider?: I18nProvider }) => {
  const [submittedData, setSubmittedData] = React.useState<any>();
  return (
    <CoreAdminContext i18nProvider={i18nProvider}>
      <Form onSubmit={(data) => setSubmittedData(data)} record={{ id: 1, field1: 'bar', field6: null }}>
        <Input source="defaultMessage" validate={required()} />
        <Input source="customMessage" validate={required('This field is required')} />
        <Input source="customMessageTranslationKey" validate={required('app.validation.required')} />
        <Input source="missingCustomMessageTranslationKey" validate={required('app.validation.missing')} />
        <button type="submit">Submit</button>
      </Form>
      <pre>{JSON.stringify(submittedData, null, 2)}</pre>
    </CoreAdminContext>
...
export const ZodResolver = ({ i18nProvider = defaultI18nProvider }: { i18nProvider?: I18nProvider }) => {
  const [result, setResult] = React.useState<any>();
  return (
    <CoreAdminContext i18nProvider={i18nProvider}>
      <Form record={{}} onSubmit={(data) => setResult(data)} resolver={zodResolver(zodSchema)}>
        <Input source="defaultMessage" />
        <Input source="customMessage" />
        <Input source="customMessageTranslationKey" />
        <Input source="missingCustomMessageTranslationKey" />
...
export const WarnWhenUnsavedChanges = ({ i18nProvider = defaultI18nProvider }: { i18nProvider?: I18nProvider }) => (
  <CoreAdminContext i18nProvider={i18nProvider}>
    <Routes>
...
export const InNonDataRouter = ({ i18nProvider = defaultI18nProvider }: { i18nProvider?: I18nProvider }) => (
...
MultiRoutesForm.argTypes = {
  url: {
    options: ['unmodified', 'modified with location state', 'modified with location search'],
    mapping: {
...
const Tab = ({ children, name }: { children: React.ReactNode; name: string }) => {
  const location = useLocation();
