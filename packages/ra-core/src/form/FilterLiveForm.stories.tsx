...
import { FilterLiveForm, FilterLiveFormProps, useInput, required, InputProps } from '.';
...
export default { title: '@strato-admin/ra-core/form/FilterLiveForm' };
...
const TextInput = ({ defaultValue = '', style, ...props }: InputProps & { style?: React.CSSProperties }) => {
  ...
  return (
    ...
    >
      <label htmlFor={`id-${field.name}`}>{props.label || field.name}</label>
  ...
  return (
    <TestMemoryRouter>
      <CoreAdminContext dataProvider={dataProvider} store={memoryStore()}>
        <ListBase resource="posts" filterDefaultValues={{ category: 'deals' }} {...props.ListBaseProps}>
...
export const ParseFormat = (props: Partial<FilterLiveFormProps>) => {
  return (
    <TestMemoryRouter>
      <CoreAdminContext dataProvider={dataProvider} store={memoryStore()}>
        <ListBase resource="posts" filterDefaultValues={{ category: 'deals' }}>
          <FilterLiveForm {...props}>
            <p>Expect a number (larger than 13 characters to trigger format)</p>
...
export const NoDebounce = () => <Basic FilterLiveFormProps={{ debounce: false }} />;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <input id="id_body" type="text" value={body || ''} onChange={onChange} />
...
export const WithExternalChanges = () => {
  ...
  return (
    <TestMemoryRouter>
      <input id="id_mounted" type="checkbox" onChange={onToggle} checked={mounted} />
      ...
      {mounted && (
        <CoreAdminContext dataProvider={dataProvider} store={memoryStore()}>
          <ListBase resource="posts">
            {' '}
            <FilterLiveForm>
              <TextInput source="title" style={{ flexDirection: 'row' }} />
              <ClearFiltersButton />
            </FilterLiveForm>
            <ExternalList />
            <FilterValue />
