...
Sets the notification message shown after a single record is deleted via [`<DeleteButton>`](./buttons/DeleteButton.md). Accepts a plain string, a translated string key, or a ReactNode (e.g. a [`<Message>`](./i18n/Message.md) component for ICU formatting).
